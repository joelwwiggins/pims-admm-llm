...
def main(argv: list[str] | None = None) -> int:
    ...
    if ph.get("offline_tf_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_ready"):
        readiness_bits.append("case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold")
    ...
    bundle_scaffold_note = (
        "bundle companion execution scaffold packaged "
        "(scaffold_present; bundle_shipped=false; bundle_ship_allowed=false; "
        "land not executed; companion not order_hint primary; first_blocking still isolation; "
        "scaffold≠bundle; dual_recovery_path=None; dual-ban; not VERDICT; not wire; not dual_linf proven)"
        if ph.get("offline_tf_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_ready")
        else "no case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold packaging flag"
    )
    print(
        ...
        f"{blueprint_note}; {first_blocker_prep_note}; {form_label_second_coreq_prep_note}; {path_third_coreq_prep_note}; {dual_linf_fourth_coreq_prep_note}; {wire_fifth_coreq_prep_note}; {bundle_companion_prep_note}; {iso_rewrite_scaffold_note}; {form_label_scaffold_note}; {dual_linf_scaffold_note}; {wire_scaffold_note}; {bundle_scaffold_note})"
    )
