...
| Sheet | Role |
|-------|------|
| How_to_read | Planner guide (three-path + dual PRIMARY/SECONDARY + `tf_offline_units` / `tf_offline_priced` / `tf_offline_timing`) |
| Submodel_Index | Unit map + **OFFLINE_TF** readiness row (FCC+COKER+CDU kernels + **priced residual readiness** + **block-solve timing readiness** — **not** classic Case 1 solve; prices ≠ duals; timings ≠ Case 1 wall) |
...
| Calc_Check | Identity / feasibility checks + **honesty audits** (`form_classic_2block`, `dual_gate_online_only`, `offline_tf_not_on_case1`, `offline_tf_priced_not_duals`, `offline_tf_timing_not_case1`) |
| Summary / Rates / Shadows | Verdict, rates, mono vs online-λ vs recovered duals; Summary honesty strip includes form + dual_gate + offline_tf units/priced/timing readiness notes |

`model.form == classic_2block_excel_path` — Excel solve remains CDU+Blender only; FCC/Coker tabs are teaching/export from `base_delta`. Offline TF kernels (FCC+COKER+CDU) plus priced residual + block-solve timing harnesses are readiness-only; not on Case 1 solve; `dual_recovery_path=None` on TF surface; not wire shipped.

After dual-honesty packaging, TF offline multi-unit registry, priced residual, and timing readiness: Index / Summary / Calc_Check **glance-lock** offline TF readiness package (units + priced + timing) + dual PRIMARY gate + classic form via static pure formatters (see `meta.planner_honesty` and `format_planner_honesty_package`). Still isolation-safe: no TF import on Excel path.
