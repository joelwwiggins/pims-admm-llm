...
After dual-honesty packaging, TF offline multi-unit registry, priced residual, timing readiness, ADMM residual harness, and ADMM block subproblem maximizer: Index / Summary / Calc_Check **glance-lock** offline TF readiness package (units + priced + timing + ADMM residual + **block subproblem** + ladder through gate-criteria + **isolation-rewrite design** + **wire-ship acceptance design**) + dual PRIMARY gate + classic form via static pure formatters (see `meta.planner_honesty` and `format_planner_honesty_package`). Isolation design packaging: design_present; rewrite_shipped=false; isolation checklist open; dual-ban — packaging ≠ rewrite shipped ≠ wire ≠ VERDICT. Wire-ship design packaging: design_present; ship_allowed=false; wire_shipped=false; dual-ban — packaging ≠ ship allow ≠ wire shipped ≠ VERDICT. Still isolation-safe: no TF import on Excel path; still not wire.
