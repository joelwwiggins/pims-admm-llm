...
- [ ] Isolation tests (`test_tf_import_isolation.py`) must be **rewritten with** the wire — not silently broken or deleted (design-only contract documents rewrite-not-delete; rewrite itself still not shipped)
