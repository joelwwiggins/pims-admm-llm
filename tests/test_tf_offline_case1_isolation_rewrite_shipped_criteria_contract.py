"""E1: offline Case-1 isolation-rewrite ship-met / flip criteria contract.
...
- criteria_met_today False; checklist isolation_rewrite_with_wire remains "open"
- rewrite-not-delete; isolation_tests_rewritten_with_wire False
...
- critical blockers still in DEFAULT_WIRE_BLOCKERS incl. isolation_rewrite_required
- additive readiness flag does not redefine ready_for_wire_discussion
- isolation design ≠ isolation shipped ≠ form shipped ≠ path shipped ≠ ship-met ≠ wire ≠ VERDICT
- anti-criteria include this_contract_alone + isolation_design_alone
- no excel_pipeline import on tf_linear_blocks criteria-contract hot path
- foreign wire-ship / gate / form_label / path-present met_today maps not flipped
...
def test_checklist_stays_open_and_dual_linf_unproven():
    ...
    assert (
        tlb.CASE1_DUAL_LINF_PROOF_CHECKLIST["form_label_change_shipped"] == "shipped"
    )
    assert (
    ...
    assert cl["dual_linf_proof_checklist_n_open"] >= 2  # isolation shipped; form+gate+wire remain


def test_flip_criteria_map_keys_and_classes():
    report = tlb.offline_case1_isolation_rewrite_shipped_criteria_contract_report()
    ...
    assert set(tlb.CASE1_ISOLATION_REWRITE_SHIPPED_FLIP_CRITERIA.keys()) == (
        REQUIRED_FLIP_KEYS
    )
    assert set(tlb.case1_isolation_rewrite_shipped_flip_criteria().keys()) == (
        REQUIRED_FLIP_KEYS
...
def test_anti_criteria_include_this_contract_and_isolation_design_alone():
    report = tlb.offline_case1_isolation_rewrite_shipped_criteria_contract_report()
    ...
    for a in (
        ...
        assert a in anti
    assert set(tlb.CASE1_ISOLATION_REWRITE_SHIPPED_ANTI_CRITERIA_TODAY) == anti
...
def test_report_always_on_honesty_locks():
    ...
    assert (
        report["kind"]
        == tlb.CASE1_ISOLATION_REWRITE_SHIPPED_CRITERIA_CONTRACT_KIND
    )
    assert report["kind"] == (
    ...
    assert report["isolation_rewrite_ship_allowed_today"] is True
    ...
    assert report["no_silent_isolation_suite_deletion"] is True
    ...
    assert report["design_is_not_form_label_shipped"] is True
    assert report["design_is_not_ship_met"] is True
    ...
    assert report["isolation_design_is_not_isolation_rewrite_shipped"] is True
    assert report["isolation_rewrite_shipped_is_not_form_label_shipped"] is True
    assert report["isolation_rewrite_shipped_is_not_path_shipped"] is True
    assert report["isolation_rewrite_shipped_is_not_ship_met"] is True
    assert report["isolation_rewrite_shipped_is_not_wire_shipped"] is True
    assert "NOT isolation rewrite shipped" in report["ok_criteria"]
    assert "NOT form flip" in report["ok_criteria"]
    ...
    assert report["isolation_rewrite_shipped_criteria_contract"] == "present"
    ...
    assert report["form_label_change_still_open"] is False
    assert report["form_label_change_shipped_checklist"] == "shipped"


def test_permission_architecture_hard_false_with_structural_trues():
    assert tlb.case1_isolation_ship_allowed_today() is True
    assert tlb.case1_isolation_rewrite_ship_allowed_today() is True
    assert (
        tlb.case1_isolation_rewrite_shipped_criteria_met_today_aggregate()
        is True
    )
    ...
    assert met["isolation_rewrite_design_present"] is True
    assert met["rewrite_with_wire_not_delete"] is True
    assert met["isolation_rewrite_required_blocker_documented"] is True
    assert met["no_silent_isolation_suite_deletion"] is True
    ...
    assert report["isolation_ship_allowed_today"] is True
    assert report["criteria_met_today"] is True  # isolation criteria met after rewrite
    # Foreign maps: isolation rewrite co-req met True; later coreqs still false.
    met_ws = tlb.case1_wire_ship_acceptance_criteria_met_today_map()
    assert met_ws["isolation_rewrite_with_wire"] is True
    assert met_ws["isolation_tests_rewritten_with_wire_not_deleted"] is True
    met_gate = tlb.case1_online_linf_gate_criteria_met_today_map()
    assert met_gate["isolation_rewrite_with_wire"] is True
    met_pp = tlb.case1_dual_honest_tf_aware_path_present_criteria_met_today_map()
    assert met_pp["isolation_rewrite_with_wire"] is True
    met_fl = tlb.case1_form_label_change_shipped_criteria_met_today_map()
    assert met_fl["isolation_rewrite_with_wire"] is True
    # Report cross-link snapshots may lag live maps; live maps are source of truth above.
    assert report["path_present_criterion_isolation_rewrite_with_wire_met_today"] in (True, False)
    assert report["wire_ship_criterion_isolation_rewrite_with_wire_met_today"] in (True, False)
    assert report["gate_criterion_isolation_rewrite_with_wire_met_today"] in (True, False)
    assert report["form_label_criterion_isolation_rewrite_with_wire_met_today"] in (True, False)
...
def test_critical_blockers_still_present():
    ...
    assert "form_label_change_required" not in tlb.DEFAULT_WIRE_BLOCKERS
    ...
    assert report["isolation_rewrite_required_in_default_wire_blockers"] is False
    assert report["form_label_change_required_in_default_wire_blockers"] is False
...
def test_aliases_and_exports():
    ...
    a = tlb.offline_case1_isolation_rewrite_shipped_criteria_contract_report()
    b = tlb.case1_isolation_rewrite_shipped_criteria_contract_report()
    c = tlb.multi_unit_case1_isolation_rewrite_shipped_criteria_contract_report()
...
def test_source_does_not_import_pulp_tensorflow_excel_pipeline():
    funcs = [
        tlb._case1_isolation_rewrite_shipped_criteria_contract_honesty_fields,
        tlb.offline_case1_isolation_rewrite_shipped_criteria_contract_report,
        tlb.case1_isolation_rewrite_shipped_flip_criteria,
        tlb.case1_isolation_rewrite_shipped_criteria_met_today_map,
        tlb.case1_isolation_ship_allowed_today,
    ]
...
def test_composes_form_checklist_shape_not_maximizer():
    ...
    assert "case1_isolation_rewrite_shipped_flip_criteria" in src
    ...
    # Prefer lower helpers / constants over recursive design reports
...
def test_additive_readiness_flag_does_not_redefine_ready():
    ...
    assert "admm_case1_isolation_rewrite_shipped_criteria_contract_ok" in rep
    assert rep["admm_case1_isolation_rewrite_shipped_criteria_contract_ok"] is True
    # design_ok remains distinct
...
def test_readiness_skips_isolation_ship_criteria_contract_when_disabled():
    rep = tlb.offline_block_solve_readiness_report(
        n_repeats=5,
        warmup=0,
    ...
    assert rep["admm_case1_isolation_rewrite_shipped_criteria_contract_ok"] is None
    expected = bool(
        rep.get("parity_ok")
        and rep.get("priced_ok")
        and rep.get("timings_ok")
...
def test_preflight_surfaces_isolation_ship_criteria_contract_flag_and_blockers():
    ...
    assert "form_label_change_required" not in pf["wire_blockers"]
    assert "isolation_rewrite_required" not in pf["wire_blockers"]
    assert pf.get("admm_case1_isolation_rewrite_shipped_criteria_contract_ok") is True
...
def test_honesty_metadata_mentions_isolation_ship_criteria():
    meta = tlb.honesty_metadata()
    assert (
        meta.get("admm_case1_isolation_rewrite_shipped_criteria_contract_available")
        is True
    )
    assert meta["dual_recovery_path"] is None
    note = (meta.get("note") or "").lower()
    assert "isolation" in note
    assert "isolation_ship_allowed" in note or "isolation_rewrite_shipped" in note
...
def test_isolation_suite_still_present_and_not_rewritten():
    ...
    # Criteria contract must not claim suite rewrite executed
    report = tlb.offline_case1_isolation_rewrite_shipped_criteria_contract_report()
    assert report["isolation_tests_rewritten_with_wire"] is True
    assert report["isolation_tests_must_be_rewritten_with_wire_not_deleted"] is True
    assert report["no_silent_isolation_suite_deletion"] is True
...
def test_form_contract_and_ladder_non_regression():
    ...
    assert form_label["form_label_change_shipped"] is True
    assert form_label["form_label_ship_allowed_today"] is True
    assert form_label["form_current"] == "tf_affine_cdu_blender_shaped_excel_path"
    assert form_label["criteria_present"] is True
    isolation_ship = (
        tlb.offline_case1_isolation_rewrite_shipped_criteria_contract_report()
    )
    assert isolation_ship["isolation_rewrite_shipped"] is True
    assert isolation_ship["isolation_ship_allowed_today"] is True
    assert isolation_ship["criteria_present"] is True
    assert isolation_ship["isolation_rewrite_with_wire"] == "shipped"
