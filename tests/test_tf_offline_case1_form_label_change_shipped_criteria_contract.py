"""E1: offline Case-1 form_label_change_shipped flip criteria contract.
...
- form_label_change_shipped False; form classic_2block_excel_path
...
- critical blockers still in DEFAULT_WIRE_BLOCKERS incl. form_label_change_required
- additive readiness flag does not redefine ready_for_wire_discussion
- form registration ≠ form_label shipped ≠ path shipped ≠ ship-met ≠ wire ≠ VERDICT
- anti-criteria include this_contract_alone + form_registration_alone + dual_space_form
- no excel_pipeline import on tf_linear_blocks criteria-contract hot path
- foreign wire-ship / gate / path-present met_today maps not flipped
...
def test_flip_criteria_map_keys_and_classes():
    ...
    assert set(tlb.CASE1_FORM_LABEL_CHANGE_SHIPPED_FLIP_CRITERIA.keys()) == (
        REQUIRED_FLIP_KEYS
    )
    assert set(tlb.case1_form_label_change_shipped_flip_criteria().keys()) == (
        REQUIRED_FLIP_KEYS
...
def test_anti_criteria_include_this_contract_and_form_registration_alone():
    report = tlb.offline_case1_form_label_change_shipped_criteria_contract_report()
    ...
    for a in (
        ...
        assert a in anti
    assert set(tlb.CASE1_FORM_LABEL_CHANGE_SHIPPED_ANTI_CRITERIA_TODAY) == anti
...
def test_report_always_on_honesty_locks():
    ...
    assert (
        report["kind"]
        == tlb.CASE1_FORM_LABEL_CHANGE_SHIPPED_CRITERIA_CONTRACT_KIND
    )
    assert report["kind"] == (
    ...
    assert report["explicit_form_mutation_path_named"] == (
    ...
    assert report["form_registration_is_not_form_label_shipped"] is True
    assert report["form_label_shipped_is_not_path_shipped"] is True
    assert report["form_label_shipped_is_not_ship_met"] is True
    assert report["form_label_shipped_is_not_wire_shipped"] is True
    ...
    assert report["form_label_change_shipped_criteria_contract"] == "present"
...
def test_permission_architecture_hard_false_with_structural_trues():
    assert tlb.case1_form_label_ship_allowed_today() is True
    assert (
        tlb.case1_form_label_change_shipped_criteria_met_today_aggregate()
        is True
    )
    met = tlb.case1_form_label_change_shipped_criteria_met_today_map()
    # Structural labels may be True; co-reqs False; aggregate still False
    assert met["planned_form_registered"] is True
    assert met["planned_form_distinct_from_classic"] is True
    assert met["form_label_change_required_blocker_documented"] is True
    assert met["explicit_form_mutation_path_named"] is True
    ...
    assert report["form_label_ship_allowed_today"] is True
    assert report["criteria_met_today"] is True
    # Foreign maps still false for form_label_change_shipped
    met_ws = tlb.case1_wire_ship_acceptance_criteria_met_today_map()
    assert met_ws["form_label_change_shipped"] is True
    met_gate = tlb.case1_online_linf_gate_criteria_met_today_map()
    assert met_gate["form_label_change_shipped"] is True
    met_pp = tlb.case1_dual_honest_tf_aware_path_present_criteria_met_today_map()
    assert met_pp["form_label_change_shipped"] is True
    assert report["wire_ship_criterion_form_label_change_shipped_met_today"] is False  # design does not flip foreign met_today
    assert report["gate_criterion_form_label_change_shipped_met_today"] is False
    assert report["path_present_criterion_form_label_change_shipped_met_today"] is False
...
def test_aliases_and_exports():
    ...
    a = tlb.offline_case1_form_label_change_shipped_criteria_contract_report()
    b = tlb.case1_form_label_change_shipped_criteria_contract_report()
    c = tlb.multi_unit_case1_form_label_change_shipped_criteria_contract_report()
...
def test_source_does_not_import_pulp_tensorflow_excel_pipeline():
    funcs = [
        tlb._case1_form_label_change_shipped_criteria_contract_honesty_fields,
        tlb.offline_case1_form_label_change_shipped_criteria_contract_report,
        tlb.case1_form_label_change_shipped_flip_criteria,
        tlb.case1_form_label_change_shipped_criteria_met_today_map,
        tlb.case1_form_label_ship_allowed_today,
    ]
...
def test_composes_form_checklist_shape_not_maximizer():
    ...
    assert "case1_form_label_change_shipped_flip_criteria" in src
...
def test_additive_readiness_flag_does_not_redefine_ready():
    ...
    assert "admm_case1_form_label_change_shipped_criteria_contract_ok" in rep
    assert rep["admm_case1_form_label_change_shipped_criteria_contract_ok"] is True
    expected = bool(
        rep.get("parity_ok")
...
def test_readiness_skips_form_label_criteria_contract_when_disabled():
    rep = tlb.offline_block_solve_readiness_report(
        n_repeats=5,
        warmup=0,
    ...
    assert rep["admm_case1_form_label_change_shipped_criteria_contract_ok"] is None
    expected = bool(
        rep.get("parity_ok")
        and rep.get("priced_ok")
        and rep.get("timings_ok")
...
def test_preflight_surfaces_form_label_criteria_contract_flag_and_blockers():
...
def test_honesty_metadata_mentions_form_label_criteria():
    meta = tlb.honesty_metadata()
    assert (
        meta.get("admm_case1_form_label_change_shipped_criteria_contract_available")
        is True
    )
    assert meta["dual_recovery_path"] is None
    note = (meta.get("note") or "").lower()
    assert "form_label_change_shipped" in note or "form_label" in note
    assert "form_label_ship_allowed" in note or "form_label_change_shipped" in note
...
def test_form_contract_and_ladder_non_regression():
    ...
    assert crit["criteria_met_today"] is False  # foreign path/wire criteria still false
    ...
    assert design["criteria_met_today"] is False  # foreign design contracts still not met for ship
    ...
    assert ws["criteria_met_today_map"]["form_label_change_shipped"] is True
    ...
    assert ship_met["criteria_met_today_map"]["form_label_change_shipped"] is True
