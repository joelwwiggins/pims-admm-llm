"""E2/E1: offline Case-1 dual_linf fourth-coreq *execution scaffold*.

Always-on sections run without TensorFlow and without PuLP / excel_pipeline on
the scaffold hot path. Locks:
- scaffold_present / execution_scaffold_present / dual_linf_scaffold_present True
...
- first_blocking_coreq = isolation_rewrite_with_wire; order_hint_index=3
...
- scaffold ≠ dual_linf proven ≠ dual_linf criteria ≠ dual_linf operational prep
  ≠ online_linf_gate criteria ≠ probe/bridge/warmstart ≠ form scaffold
  ≠ isolation scaffold ≠ path scaffold ≠ wire fifth prep ≠ VERDICT
"""
...
def test_report_always_on_honesty_locks():
    ...
    assert (
        report["kind"]
        == tlb.CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_KIND
    )
    assert report["kind"] == (
    ...
    assert (
        report["proof_composition_path_name"]
        == tlb.CASE1_DUAL_LINF_PROOF_COMPOSITION_PATH_NAME
    )
    assert report["scaffold_is_not_dual_linf_proven"] is True
    assert report["scaffold_is_not_dual_linf_proof_allow"] is True
    ...
    assert report["this_scaffold_alone_is_not_dual_linf_proof"] is True
    ...
    assert report["distinct_from_dual_linf_under_wire_criteria_contract"] is True
    assert report["distinct_from_dual_linf_fourth_coreq_operational_prep"] is True
...
def test_proof_composition_inventory_pieces_not_executed():
    inv = tlb.case1_dual_linf_execution_scaffold_proof_composition_inventory()
    ...
    assert (
        inv["proof_composition_path_name"]
        == tlb.CASE1_DUAL_LINF_PROOF_COMPOSITION_PATH_NAME
    )
    assert inv["proof_composition_path_executed_today"] is False
    ...
    assert inv["inventory_is_not_dual_linf_proven"] is True
    ...
    for need in (
        ...
        "online_linf_gate_criteria_gate_open",
        "probe_diagnostic_linf",
        "bridge_diagnostic_linf",
        "warmstart_diagnostic_linf",
        "planned_dual_recovery_path_under_wire",
        "dual_linf_proof_checklist_open_ids",
        "named_proof_composition_path",
    ):
        assert need in ids


def test_scaffold_steps_do_not_execute_dual_linf_proof():
    steps = tlb.case1_dual_linf_fourth_coreq_scaffold_steps()
    assert len(steps) >= 8
    assert all(s["executes_dual_linf_proof"] is False for s in steps)
    assert all(s.get("proves_dual_linf") is False for s in steps)
    assert all(s.get("proof_composition_path_executed") is False for s in steps)
    ids = [s["step_id"] for s in steps]
    assert "dual_linf_criteria_documented" in ids
    assert "online_linf_gate_criteria_documented_gate_open" in ids
    assert "probe_bridge_warmstart_documented_diagnostic_only" in ids
    ...
    assert "order_hint_index_3" in ids
    assert "is_first_blocking_coreq_false" in ids
    assert "scaffold_complete_is_not_dual_linf_proven" in ids
...
def test_aliases_and_kind():
    a = tlb.offline_case1_dual_linf_fourth_coreq_execution_scaffold_report()
    b = tlb.offline_case1_dual_linf_execution_scaffold_report()
    c = tlb.case1_dual_linf_fourth_coreq_execution_scaffold_report()
    d = tlb.multi_unit_case1_dual_linf_fourth_coreq_execution_scaffold_report()
...
def test_distinct_from_prep_criteria_form_iso_path_scaffold():
    sc = tlb.offline_case1_dual_linf_fourth_coreq_execution_scaffold_report()
...
def test_readiness_additive_flag_does_not_redefine_ready():
    r = tlb.offline_block_solve_readiness_report(
        n_repeats=5,
        warmup=0,
        include_admm_case1_dual_linf_fourth_coreq_execution_scaffold=True,
    )
    assert r["admm_case1_dual_linf_fourth_coreq_execution_scaffold_ok"] is True
    ready = bool(
        r.get("parity_ok")
        and r.get("priced_ok")
    ...
    r_off = tlb.offline_block_solve_readiness_report(
        n_repeats=5,
        warmup=0,
        include_admm_case1_dual_linf_fourth_coreq_execution_scaffold=False,
    )
    assert r_off["admm_case1_dual_linf_fourth_coreq_execution_scaffold_ok"] is None
...
def test_preflight_additive_flag():
    ...
    assert pf["admm_case1_dual_linf_fourth_coreq_execution_scaffold_ok"] is True
...
def test_source_purity_no_excel_pulp_tf_on_scaffold_hot_path():
    src = inspect.getsource(
        tlb.offline_case1_dual_linf_fourth_coreq_execution_scaffold_report
    )
    src_h = inspect.getsource(
        tlb._case1_dual_linf_fourth_coreq_execution_scaffold_honesty_fields
    )
    blob = src + src_h
    ...
    assert "dual_linf_proof_allowed_today = True" not in blob
    assert "gate_flip_allowed_today = True" not in blob
...
def test_feasibility_scaffold_present_does_not_allow_dual_linf_proof():
    report = tlb.offline_case1_dual_linf_fourth_coreq_execution_scaffold_report()
    assert report["scaffold_present"] is True
    assert report["dual_linf_under_wire_status"] == "unproven"
    assert report["dual_linf_proof_allowed_today"] is False
    assert report["gate_flip_allowed_today"] is False
    assert report["proof_composition_path_executed_today"] is False
    # AND-lock: scaffold present must not co-exist with dual_linf proven / proof allow
    assert not (
        report["scaffold_present"] is True
        and report["dual_linf_under_wire_status"] == "proven"
    )
    assert not (
        report["scaffold_present"] is True
        and report["dual_linf_proof_allowed_today"] is True
    )
    assert not (
        report["scaffold_present"] is True
        and report["gate_flip_allowed_today"] is True
    )
    assert not (
        report["scaffold_present"] is True
        and report["proof_composition_path_executed_today"] is True
    )
