"""E1/E2: offline Case-1 dual-honest TF-aware path *execution scaffold*.
...
- scaffold_present / execution_scaffold_present True
...
- scaffold ≠ path shipped ≠ ship-met ≠ wire ≠ bundle ≠ isolation rewrite shipped
  ≠ form ship ≠ VERDICT ≠ dual L∞ under wire proof
- order_hint is not an executor; no auto-wire
- no excel_pipeline / pulp / tensorflow import on scaffold hot path
...
def test_report_always_on_honesty_locks():
    report = tlb.offline_case1_dual_honest_tf_aware_path_execution_scaffold_report()
    assert (
        report["kind"]
        == tlb.CASE1_DUAL_HONEST_TF_AWARE_PATH_EXECUTION_SCAFFOLD_KIND
    )
    assert report["kind"] == "offline_case1_dual_honest_tf_aware_path_execution_scaffold"
    ...
    assert report["scaffold_is_not_path_shipped"] is True
    assert report["scaffold_is_not_path_present_for_ship"] is True
    ...
    assert report["scaffold_is_not_ship_allow"] is True
    ...
    assert report["this_scaffold_alone_is_not_multi_blocker_ship"] is True
    ...
    assert report["scaffold_does_not_flip_dual_honest_tf_aware_path_present_met_today"] is True
...
def test_compose_path_pieces_finite_and_honest():
    report = tlb.offline_case1_dual_honest_tf_aware_path_execution_scaffold_report()
    compose = report["compose"]
    assert compose["pieces_present"] is True
    pieces = compose["pieces"]
    assert pieces["cdu_affine"] is True
    assert pieces["blender_pooling"] is True
    assert pieces["stream_alignment"] is True
    assert pieces["no_blender_units"] is True
    assert list(compose["streams"]["names"]) == list(tlb.CASE1_SHAPED_LINKING_STREAMS)
    assert compose["surfaces"]["cdu_surface"] == "offline_affine_base_delta"
    assert compose["surfaces"]["blender_surface"] == "linear_quality_pooling"
    assert compose["streams"]["cdu_intermediates_finite"] is True
    assert compose["streams"]["blender_use_finite"] is True
    assert compose["path_shipped"] is False
    assert compose["wire_shipped"] is False
    assert compose["bundle_shipped"] is False
    assert compose["is_not_maximizer_sla"] is True
    assert compose["is_not_residual_must_vanish"] is True
    # labeled λ default fixture path
    assert compose["lambda_meta"]["live_lambda_used"] is True
    assert compose["lambda_meta"]["live_lambda_source"] == tlb.LIVE_LAMBDA_SOURCE_FIXTURE
    assert compose["lambda_meta"]["dual_recovery_path"] is None
    diag = compose["diagnostic"]
    assert diag["diagnostic_only"] is True
    assert diag["is_not_dual_linf_under_wire_proof"] is True
    assert diag["is_not_verdict"] is True
    assert diag["dual_linf_under_wire_status"] == "unproven"
    assert diag["stream_linf"] is None or isinstance(diag["stream_linf"], float)


def test_unlabeled_lambda_rejected():
    report = tlb.offline_case1_dual_honest_tf_aware_path_execution_scaffold_report(
        live_lambda={"naphtha": 1.0, "distillate": 2.0, "gasoil": 3.0, "residue": 4.0},
        live_lambda_source=None,
        include_diagnostic=True,
    )
    assert report["ok"] is True
    meta = report["compose"]["lambda_meta"]
    assert meta["live_lambda_rejected_unlabeled"] is True
    assert meta["live_lambda_used"] is False
    assert meta["dual_recovery_path"] is None
    assert report["dual_recovery_path"] is None
    assert report["dual_linf_under_wire_status"] == "unproven"


def test_labeled_caller_lambda_accepted_diagnostic_only():
    lam = {"naphtha": 1.0, "distillate": 0.0, "gasoil": 0.0, "residue": 0.0}
    report = tlb.offline_case1_dual_honest_tf_aware_path_execution_scaffold_report(
        live_lambda=lam,
        live_lambda_source=tlb.LIVE_LAMBDA_SOURCE_CALLER_SUPPLIED,
        include_diagnostic=True,
    )
    assert report["ok"] is True
    meta = report["compose"]["lambda_meta"]
    assert meta["live_lambda_used"] is True
    assert meta["live_lambda_source"] == tlb.LIVE_LAMBDA_SOURCE_CALLER_SUPPLIED
    assert report["dual_recovery_path"] is None
    assert report["dual_linf_under_wire_status"] == "unproven"
    # even finite L∞ does not prove dual_linf under wire
    diag = report["compose"]["diagnostic"]
    assert diag["is_not_dual_linf_under_wire_proof"] is True
    assert diag["dual_linf_under_wire_status"] == "unproven"
...
def test_units_no_silent_blender():
    ...
    assert report["scaffold_pieces"]["no_blender_units"] is True
...
def test_aliases_and_exports():
    ...
    a = tlb.offline_case1_dual_honest_tf_aware_path_execution_scaffold_report()
    b = tlb.case1_dual_honest_tf_aware_path_execution_scaffold_report()
    c = tlb.multi_unit_case1_dual_honest_tf_aware_path_execution_scaffold_report()
...
def test_source_does_not_import_pulp_tensorflow_excel_pipeline():
    funcs = [
        tlb._case1_dual_honest_tf_aware_path_execution_scaffold_honesty_fields,
        tlb.case1_dual_honest_tf_aware_path_execution_scaffold_compose,
        tlb.offline_case1_dual_honest_tf_aware_path_execution_scaffold_report,
    ]
...
def test_compose_reuses_helpers_not_maximizer_sla():
    src = inspect.getsource(
        tlb.case1_dual_honest_tf_aware_path_execution_scaffold_compose
    )
    assert "case1_dual_honest_tf_aware_path_shape" in src
    assert "project_cdu_y_to_case1_intermediates" in src
    assert "blender_recipe_use_from_products" in src
    assert "cached_offline_unit_coeffs" in src
    # must not re-run maximizer / skeleton SLA
    assert "offline_case1_shaped_cdu_blender_linking_report" not in src
    assert "case1_shaped_cdu_blender_linking_round" not in src
    # no residual-must-vanish
    assert "residual_must_vanish" not in src or "is_not_residual_must_vanish" in src
...
def test_additive_readiness_flag_does_not_redefine_ready():
    ...
    assert "admm_case1_dual_honest_tf_aware_path_execution_scaffold_ok" in rep
...
def test_readiness_skips_scaffold_when_disabled():
    rep = tlb.offline_block_solve_readiness_report(
        n_repeats=5,
        warmup=0,
    ...
    assert rep["admm_case1_dual_honest_tf_aware_path_execution_scaffold_ok"] is None
    expected = bool(
        rep.get("parity_ok")
        and rep.get("priced_ok")
        and rep.get("timings_ok")
...
def test_preflight_surfaces_scaffold_flag_and_blockers():
...
def test_honesty_metadata_mentions_scaffold():
...
def test_isolation_suite_file_not_modified_this_cycle():
    ...
    assert path.is_file()
    # Presence only: do not rewrite/delete; co-run is the real gate in CI plan.
