"""E1: offline Case-1 dual-honest multi-blocker wire bundle design contract.
...
- criteria_met_today False; isolation_rewrite_shipped False; isolation checklist open
...
- order_hint is not executor; no auto-wire
...
- bundle design ≠ wire-ship acceptance alone ≠ isolation/form/path criteria alone
...
def test_report_always_on_honesty_locks():
    ...
    assert (
        report["kind"]
        == tlb.CASE1_DUAL_HONEST_MULTI_BLOCKER_WIRE_BUNDLE_DESIGN_CONTRACT_KIND
    )
    assert (
        report["kind"]
    ...
    assert report["bundle_criteria_met_today"] is False
    ...
    assert report["design_is_not_bundle_shipped"] is True
    assert report["design_is_not_bundle_ship_allow"] is True
    ...
    assert report["design_is_not_form_label_change_shipped"] is True
    ...
    assert report["wire_ship_acceptance_design_alone_is_not_bundle_ship"] is True
    ...
    assert "NOT bundle ship allow" in report["ok_criteria"] or "NOT ship allow" in report[
        "ok_criteria"
    ]
    ...
    assert (
        report["bundle_name"]
        == tlb.SUGGESTED_NEXT_WAVE_AFTER_PREFLIGHT
...
def test_members_map_keys_and_classes():
    report = tlb.offline_case1_dual_honest_multi_blocker_wire_bundle_design_contract_report()
    members = report["members_map"]
    assert set(members.keys()) == REQUIRED_MEMBER_KEYS
    assert (
        members["online_linf_gate_under_tf_path"]
        == tlb.FLIP_CRITERION_REQUIRED_UNDER_WIRE_ONLY
    )
    assert (
        members["dual_linf_under_wire_proven"]
        == tlb.FLIP_CRITERION_REQUIRED_UNDER_WIRE_ONLY
    )
    for k in REQUIRED_MEMBER_KEYS:
        if k not in (
            "online_linf_gate_under_tf_path",
            "dual_linf_under_wire_proven",
        ):
            assert members[k] == tlb.FLIP_CRITERION_REQUIRED
    ...
    for a in (
        ...
        assert a in anti
    assert (
        set(tlb.CASE1_DUAL_HONEST_MULTI_BLOCKER_WIRE_BUNDLE_MEMBERS.keys())
        == REQUIRED_MEMBER_KEYS
    )
    assert (
        set(tlb.case1_dual_honest_multi_blocker_wire_bundle_members().keys())
    ...
    assert report["members_formalized"] is True
    assert report["order_hint_is_not_executor"] is True
    assert report["no_auto_wire"] is True
    assert len(report["order_hint"]) >= 3
    assert tlb.CASE1_DUAL_HONEST_MULTI_BLOCKER_WIRE_BUNDLE_ORDER_HINT_IS_NOT_EXECUTOR is True
    assert tlb.CASE1_DUAL_HONEST_MULTI_BLOCKER_WIRE_BUNDLE_NO_AUTO_WIRE is True


def test_ship_permission_hard_false():
    assert tlb.case1_dual_honest_multi_blocker_wire_bundle_ship_allowed_today() is False
    assert (
        tlb.case1_dual_honest_multi_blocker_wire_bundle_criteria_met_today_aggregate()
        is False
    )
    assert tlb.case1_dual_honest_multi_blocker_wire_bundle_shipped() is False
    met = tlb.case1_dual_honest_multi_blocker_wire_bundle_member_status_today()
...
def test_aliases_and_exports():
    ...
    a = tlb.offline_case1_dual_honest_multi_blocker_wire_bundle_design_contract_report()
    b = tlb.case1_dual_honest_multi_blocker_wire_bundle_design_contract_report()
    c = tlb.multi_unit_case1_dual_honest_multi_blocker_wire_bundle_design_contract_report()
...
def test_source_does_not_import_pulp_tensorflow_excel_pipeline():
    funcs = [
        tlb._case1_dual_honest_multi_blocker_wire_bundle_design_contract_honesty_fields,
        tlb.offline_case1_dual_honest_multi_blocker_wire_bundle_design_contract_report,
        tlb.case1_dual_honest_multi_blocker_wire_bundle_members,
        tlb.case1_dual_honest_multi_blocker_wire_bundle_member_status_today,
...
def test_composes_checklist_form_not_maximizer():
    ...
    # Prefer lower helpers / constants over recursive isolation/wire-ship reports
    ...
    # No auto-executor symbols
    assert "execute_suggested_next_wave" not in src
    assert "auto_wire" not in src or "no_auto_wire" in src
...
def test_additive_readiness_flag_does_not_redefine_ready():
    ...
    assert "admm_case1_dual_honest_multi_blocker_wire_bundle_design_contract_ok" in rep
    assert (
...
def test_honesty_metadata_mentions_bundle_design():
    meta = tlb.honesty_metadata()
    assert (
        meta.get(
            "admm_case1_dual_honest_multi_blocker_wire_bundle_design_contract_available"
        )
        is True
    )
    ...
    assert "multi-blocker" in note or "multi_blocker" in note
...
def test_form_contract_and_ladder_non_regression():
    ...
    assert iso_ship["isolation_rewrite_shipped"] is False
    assert iso_ship.get("isolation_ship_allowed_today", False) is False
    # form still classic
    assert tlb.CASE1_FORM_CURRENT == "classic_2block_excel_path"
    # bundle design does not allow ship or claim wire/bundle
    bd = tlb.offline_case1_dual_honest_multi_blocker_wire_bundle_design_contract_report()
    assert bd["bundle_ship_allowed_today"] is False
    assert bd["bundle_shipped"] is False
    assert bd["wire_shipped"] is False
    assert bd["isolation_rewrite_shipped"] is False
    assert bd["dual_linf_under_wire_status"] == "unproven"
