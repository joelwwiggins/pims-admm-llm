"""E1: offline Case-1 dual_honest_tf_aware_path_present ship-met criteria contract.
...
- wire_ship_allowed_today False; gate_flip_allowed_today False
...
- criteria ≠ ship-met ≠ path shipped ≠ wire shipped ≠ ship allow ≠ VERDICT
- anti-criteria include this_ship_met_criteria_contract_alone + path_design_alone
- no excel_pipeline import on tf_linear_blocks criteria-contract hot path
- foreign wire-ship / gate met_today maps not flipped
...
def test_flip_criteria_map_keys_and_classes():
    ...
    assert set(tlb.CASE1_DUAL_HONEST_TF_AWARE_PATH_PRESENT_FLIP_CRITERIA.keys()) == (
        REQUIRED_FLIP_KEYS
    )
    assert set(tlb.case1_dual_honest_tf_aware_path_present_flip_criteria().keys()) == (
        REQUIRED_FLIP_KEYS
...
def test_anti_criteria_include_this_contract_and_path_design_alone():
    report = tlb.offline_case1_dual_honest_tf_aware_path_present_criteria_contract_report()
    ...
    for a in (
        ...
        assert a in anti
    assert set(tlb.CASE1_DUAL_HONEST_TF_AWARE_PATH_PRESENT_ANTI_CRITERIA_TODAY) == anti
...
def test_report_always_on_honesty_locks():
    ...
    assert (
        report["kind"]
        == tlb.CASE1_DUAL_HONEST_TF_AWARE_PATH_PRESENT_CRITERIA_CONTRACT_KIND
    )
    assert report["kind"] == (
    ...
    assert report["path_design_present_is_not_ship_met"] is True
    assert report["ship_met_is_not_path_shipped"] is True
    assert report["ship_met_is_not_wire_shipped"] is True
    assert "NOT ship-met" in report["ok_criteria"]
    ...
    assert report["path_present_criteria_contract"] == "present"
...
def test_ship_met_permission_hard_false_with_structural_trues():
    ...
    assert met["path_shape_matches_case1_cdu_blender_package"] is True
    ...
    assert met["no_blender_affine_units_entry"] is True
...
def test_aliases_and_exports():
    ...
    a = tlb.offline_case1_dual_honest_tf_aware_path_present_criteria_contract_report()
    b = tlb.case1_dual_honest_tf_aware_path_present_criteria_contract_report()
    c = tlb.multi_unit_case1_dual_honest_tf_aware_path_present_criteria_contract_report()
...
def test_source_does_not_import_pulp_tensorflow_excel_pipeline():
    funcs = [
        tlb._case1_dual_honest_tf_aware_path_present_criteria_contract_honesty_fields,
        tlb.offline_case1_dual_honest_tf_aware_path_present_criteria_contract_report,
        tlb.case1_dual_honest_tf_aware_path_present_flip_criteria,
        tlb.case1_dual_honest_tf_aware_path_present_criteria_met_today_map,
        tlb.case1_dual_honest_tf_aware_path_present_ship_met_allowed_today,
    ]
...
def test_composes_shape_checklist_form_not_maximizer():
    ...
    assert "case1_dual_honest_tf_aware_path_present_flip_criteria" in src
...
def test_additive_readiness_flag_does_not_redefine_ready():
    ...
    assert "admm_case1_dual_honest_tf_aware_path_present_criteria_contract_ok" in rep
    assert rep["admm_case1_dual_honest_tf_aware_path_present_criteria_contract_ok"] is True
    expected = bool(
        rep.get("parity_ok")
...
def test_readiness_skips_path_present_criteria_contract_when_disabled():
    rep = tlb.offline_block_solve_readiness_report(
        n_repeats=5,
        warmup=0,
    ...
    assert rep["admm_case1_dual_honest_tf_aware_path_present_criteria_contract_ok"] is None
    expected = bool(
        rep.get("parity_ok")
        and rep.get("priced_ok")
        and rep.get("timings_ok")
...
def test_preflight_surfaces_path_present_criteria_contract_flag_and_blockers():
    ...
    assert pf.get("admm_case1_dual_honest_tf_aware_path_present_criteria_contract_ok") is True
...
def test_honesty_metadata_mentions_path_present_criteria():
    meta = tlb.honesty_metadata()
    assert (
        meta.get(
            "admm_case1_dual_honest_tf_aware_path_present_criteria_contract_available"
        )
        is True
    )
    assert meta["dual_recovery_path"] is None
    note = (meta.get("note") or "").lower()
    assert "ship-met" in note or "path_present" in note or "present-for-ship" in note
    assert "ship_met_allowed" in note or "ship-met" in note
