...
def format_tf_offline_case1_dual_honest_multi_blocker_wire_implementation_blueprint_howto() -> Dict[str, str]:
    """Static offline Case-1 dual-honest multi-blocker wire *implementation blueprint* How_to (#64 packaging).

    Planner-facing note that dual-honest multi-blocker wire *implementation
    blueprint / go-board* packaging exists: blueprint_present=true /
    implementation_blueprint_present=true / wire_go_board_present=true
    (packaging existence); first_blocking_coreq=isolation_rewrite_with_wire
    (static expected label); rehearsal_present=true / wire_rehearsal_present=true
    (linked packaging); scaffold_present=true / execution_scaffold_present=true
    (linked packaging); path_shipped=false; dual_honest_tf_aware_path_present
    ship-met=false; wire_shipped=false; bundle_shipped=false;
    bundle_ship_allowed_today=false; criteria_met_today=false;
    isolation_rewrite_shipped=false; isolation_rewrite_with_wire=open;
    ...
    gate_flip_allowed_today=false; dual_recovery_path=None today;
    dual_recovery_path_planned_when_shipped labeled honestly (not pure-ADMM);
    feature_flag reserved + enabled_today=false; go-board / order_hint /
    prep-map static labels only (not executor); first_blocking_coreq alone
    does not close checklist; multi-way dual-ban (blueprint_present ≠
    path_shipped ≠ ship-met ≠ wire_shipped ≠ bundle_shipped ≠
    isolation_rewrite_shipped ≠ form_label_change_shipped ≠ VERDICT ≠
    dual L∞ under wire proof); packaging alone / this blueprint alone /
    go_board alone / first_blocking_coreq alone / prep_map alone /
    scaffold_plus_rehearsal_plus_blueprint alone / path design alone /
    path present criteria alone / multi-blocker design alone / multi-blocker
    ship-met criteria alone / wire-ship acceptance alone / isolation/form
    criteria alone / scaffold alone / rehearsal alone are not path-ship /
    wire-ship / isolation-rewrite-ship / form-ship / ship-met / bundle-ship
    enablers; blockers remain; UNITS FCC/COKER/CDU.
    Distinct from multi-blocker *design* How_to (#57), *ship-met criteria*
    How_to (#59), *scaffold* How_to (#61), and *rehearsal* How_to (#63) —
    this is *order_hint-sequenced first-blocker + prep visibility packaging*;
    design remains *what*; criteria remains *when*; scaffold remains
    *offline how-without-ship packaging*; rehearsal remains *co-req readiness
    dry-run packaging*; TF blueprint remains the live go-board surface.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_dual_honest_multi_blocker_wire_implementation_blueprint_report.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_BLUEPRINT_ANTI_CRITERIA)
    streams = ",".join(_CASE1_SHAPED_LINKING_STREAMS)
    ...
    go_board_glance = (
        ...
        "no_blender_offline_affine_kernel=true; "
        "feature_flag_enabled_today=false; dual_recovery_path=None; "
        "order_hint_is_not_executor=true; prep_map_is_not_ship=true"
    )
    one_liner = (
        "Offline Case-1 dual-honest multi-blocker wire *implementation blueprint / "
        "go-board* readiness exists (static packaging of #64 harness): "
        f"form_current={_CASE1_FORM_CURRENT}; "
        "blueprint_present=true; implementation_blueprint_present=true; "
        ...
        f"blender_surface={_CASE1_SHAPED_BLENDER_SURFACE}; "
        f"intermediates={streams}; "
        f"go_board_status_glance={go_board_glance}; "
        f"feature_flag_name={_CASE1_PATH_DESIGN_FEATURE_FLAG_NAME}; "
        "feature_flag_enabled_today=false; "
        "dual_recovery_path=None today; "
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "blueprint_is_not_path_shipped=true; blueprint_is_not_ship_met=true; "
        "blueprint_is_not_wire=true; blueprint_is_not_bundle_shipped=true; "
        ...
        "first_blocking_coreq_alone_is_not_ship=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        "prep_map_is_not_ship=true; "
        "distinct_from_scaffold_and_rehearsal_and_bundle_design_and_ship_met_criteria_packaging=true; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        f"open checklist ({open_ids}); "
        "no_blender_offline_affine_kernel_blocker_still_true=true; "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        ...
        "prep_map_alone / scaffold_plus_rehearsal_plus_blueprint_alone / "
        "this_rehearsal_alone / this_scaffold_alone / path design alone / "
        "path-present criteria alone / multi-blocker design alone / multi-blocker "
        "ship-met criteria alone / wire_ship_acceptance_design_alone / isolation "
        "design alone / isolation ship criteria alone / form_label criteria alone / "
        "gate criteria alone are not path-ship / wire-ship / isolation-rewrite-ship / "
        "form-label-ship / path-ship / ship-met / bundle-ship enablers. Case 1 duals "
        "remain PRIMARY free online λ / SECONDARY recovered blender; wire blueprint "
        "packaging is not the Case 1 VERDICT dual gate and is not path "
        "shipped / not wire shipped / not bundle shipped / not isolation rewrite "
        "shipped / not form flip / not form_label_change_shipped / not ship-met / "
        "not ship allowed; order_hint is not an executor / not auto-wire; design "
        "formalizes *what*; ship-met criteria formalizes *when*; TF scaffold "
        "formalizes *offline how-without-ship*; TF rehearsal formalizes *co-req "
        "readiness dry-run under scaffold without ship*; TF blueprint formalizes "
        "*order_hint-sequenced first-blocker + file-level prep without ship*; this "
        "formalizes *planner-facing packaging of that blueprint* without shipping."
    )
    return {
        "topic": "tf_offline_case1_dual_honest_multi_blocker_wire_implementation_blueprint",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "blender_surface": _CASE1_SHAPED_BLENDER_SURFACE,
        "intermediates": streams,
        "go_board_status_glance": go_board_glance,
        "feature_flag_name": _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME,
        "feature_flag_enabled_today": "false",
...
def format_tf_offline_case1_dual_linf_under_wire_criteria_contract_howto() -> Dict[str, str]:
    """Static offline Case-1 dual_linf_under_wire flip-criteria contract How_to.

    Planner-facing dual-ban packaging twin of TF dual_linf flip-criteria residual:
    criteria_present=true; dual_linf_under_wire=unproven; criteria_met_today=false;
    dual_linf_proof_allowed_today=false; distinct from online_linf_gate criteria;
    anti-criteria dual-ban (probe/bridge/warmstart/packaging/online_gate-criteria ≠ proof);
    dual_recovery_path=None; first_blocking still isolation_rewrite_with_wire;
    ship flags hard false. Does **not** load tf_linear_blocks or tensorflow;
    does **not** call offline_case1_dual_linf_under_wire_criteria_contract_report.
    No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    flip_keys = ",".join(_CASE1_DUAL_LINF_UNDER_WIRE_FLIP_CRITERIA_KEYS)
    anti_keys = ",".join(_CASE1_DUAL_LINF_UNDER_WIRE_ANTI_CRITERIA)
    one_liner = (
        "Offline Case-1 dual_linf_under_wire flip-criteria contract readiness exists "
        "(static packaging): criteria_present=true; flip_criteria_formalized=true; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        "criteria_met_today=false; dual_linf_proof_allowed_today=false; "
        "contract_is_not_dual_linf_under_wire_proof=true; "
        "criteria_present_is_not_proven=true; "
        "distinct_from_online_linf_gate_criteria_contract=true "
        "(gates status dual_linf_under_wire / order_hint dual_linf_under_wire_proven, "
        "not checklist id online_linf_gate_under_tf_path); "
        f"first_blocking_coreq={_CASE1_FIRST_BLOCKING_COREQ}; "
        "online_linf_gate_under_tf_path=open; gate_flip_allowed_today=false; "
        f"form_current={_CASE1_FORM_CURRENT}; form_planned={_CASE1_FORM_PLANNED}; "
        "path_shipped=false; wire_shipped=false; bundle_shipped=false; "
        "form_label_change_shipped=false; isolation_rewrite_shipped=false; "
        "dual_recovery_path=None today; "
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        f"flip_criteria_required=[{flip_keys}]; "
        f"anti_criteria_today=[{anti_keys}] "
        "(probe/bridge/warmstart/packaging/online_gate_criteria_alone ≠ proof); "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        "does not clear wire_blockers; not dual L∞ proven under wire; not gate flip; "
        "not wire; not form flip; not VERDICT. Case 1 duals remain PRIMARY free online λ / "
        "SECONDARY recovered blender; this packaging is not dual L∞ under wire proof."
    )
    return {
        "topic": "tf_offline_case1_dual_linf_under_wire_criteria_contract",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "isolation_tests_rewritten_with_wire": "false",
        "not_wire_shipped": "true",
        "first_blocking_coreq": _CASE1_FIRST_BLOCKING_COREQ,
        "contract_is_not_dual_linf_under_wire_proof": "true",
        "contract_is_not_gate_flip": "true",
        "contract_is_not_wire": "true",
        "contract_is_not_verdict_gate": "true",
        ...
        "online_linf_gate_criteria_alone_is_not_proof": "true",
...
def format_tf_offline_case1_form_label_second_coreq_operational_prep_howto() -> Dict[str, str]:
    """Static offline Case-1 form_label second-coreq *operational prep* How_to.

    Planner-facing dual-ban packaging twin of TF form second-coreq operational prep:
    prep_present=true; form classic; form_label_change_shipped=false;
    form_label_ship_allowed=false; prep≠ship dual-ban; dual_recovery_path=None;
    first_blocking still isolation_rewrite_with_wire (form is second coreq);
    mutation path not executed; distinct from form_label ship criteria How_to.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_form_label_second_coreq_operational_prep_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_FORM_LABEL_SECOND_COREQ_PREP_ANTI_CRITERIA)
    mut = _CASE1_FORM_LABEL_MUTATION_PATH_NAME
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    one_liner = (
        "Offline Case-1 form_label second-coreq *operational prep* readiness exists "
        "(static packaging): prep_present=true; form_label_second_coreq_prep_present=true; "
        ...
        "(criteria=when shipped; this prep=how prep lands without ship); "
        "path_shipped=false; wire_shipped=false; bundle_shipped=false; "
        "isolation_rewrite_shipped=false; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        "online_linf_gate_under_tf_path=open; gate_flip_allowed_today=false; "
        ...
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "prep_is_not_form_label_change_shipped=true; prep_is_not_form_flip=true; "
        "prep_is_not_wire=true; prep_is_not_verdict_gate=true; "
        "prep_is_not_dual_linf_under_wire_proof=true; "
        "this_prep_alone_is_not_ship=true; packaging_alone_is_not_ship=true; "
        "form_label_criteria_alone_is_anti_criterion=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        "Case 1 duals remain PRIMARY free online λ / SECONDARY recovered blender; "
        "this packaging is not form_label_change_shipped / not form flip / not wire / "
        "not VERDICT dual gate / not dual L∞ under wire proof."
    )
    return {
        "topic": "tf_offline_case1_form_label_second_coreq_operational_prep",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "this_prep_alone_is_not_ship": "true",
        "packaging_alone_is_not_ship": "true",
        "packaging_is_not_form_label_change_shipped": "true",
        "packaging_is_not_wire_shipped": "true",
        "packaging_is_not_path_shipped": "true",
        "distinct_from_form_label_change_shipped_criteria_contract": "true",
        "criteria_contract_formalizes_when_shipped": "true",
        "this_prep_formalizes_how_prep_lands_without_ship": "true",
        "form_label_criteria_alone_is_anti_criterion": "true",
        "order_hint_is_not_executor": "true",
        "no_auto_wire": "true",
        "units_affine_unchanged": _OFFLINE_TF_UNITS,
...
def format_tf_offline_case1_path_third_coreq_operational_prep_howto() -> Dict[str, str]:
    """Static offline Case-1 path third-coreq *operational prep* How_to.

    Planner-facing dual-ban packaging twin of TF path third-coreq operational prep:
    prep_present=true; path_shipped=false; dual_honest_tf_aware_path_present ship-met=false;
    feature_flag=false; prep≠ship dual-ban; dual_recovery_path=None;
    first_blocking still isolation_rewrite_with_wire (path is third coreq);
    distinct from path design / path present criteria / execution scaffold How_to.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_path_third_coreq_operational_prep_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_PATH_THIRD_COREQ_PREP_ANTI_CRITERIA)
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    flag = _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME
    one_liner = (
        "Offline Case-1 path third-coreq *operational prep* readiness exists "
        "(static packaging): prep_present=true; path_third_coreq_prep_present=true; "
        "operational_prep_present=true; "
        ...
        f"first_blocking_coreq={first_blocking} (path is third coreq, not first); "
        "is_first_blocking_coreq=false; order_hint_index=2; "
        "order_hint_coreq=dual_honest_tf_aware_path_present; "
        "distinct_from_path_design_contract=true "
        "(design=what; present criteria=when present-for-ship; scaffold=compose without ship; "
        "this prep=how third-coreq prep lands without ship); "
        ...
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "prep_is_not_path_shipped=true; prep_is_not_ship_met=true; "
        "prep_is_not_wire=true; prep_is_not_verdict_gate=true; "
        "prep_is_not_dual_linf_under_wire_proof=true; "
        "prep_is_not_feature_flag_enable=true; "
        "this_prep_alone_is_not_ship=true; packaging_alone_is_not_ship=true; "
        "path_design_alone_is_anti_criterion=true; "
        "path_present_criteria_alone_is_anti_criterion=true; "
        "scaffold_alone_is_anti_criterion=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        "Case 1 duals remain PRIMARY free online λ / SECONDARY recovered blender; "
        "this packaging is not path_shipped / not dual_honest_tf_aware_path_present "
        "ship-met / not wire / not feature flag enable / not VERDICT dual gate / "
        "not dual L∞ under wire proof."
    )
    return {
        "topic": "tf_offline_case1_path_third_coreq_operational_prep",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "packaging_is_not_wire_shipped": "true",
        "distinct_from_path_design_contract": "true",
        "distinct_from_path_present_criteria_contract": "true",
        "distinct_from_path_execution_scaffold": "true",
        "path_design_formalizes_what": "true",
        "path_present_criteria_formalizes_when_present_for_ship": "true",
        "execution_scaffold_formalizes_compose_without_ship": "true",
...
def format_tf_offline_case1_dual_linf_fourth_coreq_operational_prep_howto() -> Dict[str, str]:
    """Static offline Case-1 dual_linf fourth-coreq *operational prep* How_to.

    Planner-facing dual-ban packaging twin of TF dual_linf fourth-coreq operational prep:
    prep_present=true; dual_linf unproven; proof_allowed=false; gate open;
    prep≠proof dual-ban; dual_recovery_path=None;
    first_blocking still isolation_rewrite_with_wire (dual_linf is fourth coreq);
    distinct from dual_linf criteria / online_linf_gate criteria / probe packaging.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_dual_linf_fourth_coreq_operational_prep_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_DUAL_LINF_FOURTH_COREQ_PREP_ANTI_CRITERIA)
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    flag = _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME
    one_liner = (
        "Offline Case-1 dual_linf fourth-coreq *operational prep* readiness exists "
        "(static packaging): prep_present=true; dual_linf_fourth_coreq_prep_present=true; "
        "operational_prep_present=true; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        "dual_linf_proof_allowed_today=false; criteria_met_today=false; "
        "online_linf_gate_under_tf_path=open; gate_flip_allowed_today=false; "
        f"feature_flag_name={flag}; feature_flag_enabled_today=false; "
        f"first_blocking_coreq={first_blocking} (dual_linf is fourth coreq, not first); "
        "is_first_blocking_coreq=false; order_hint_index=3; "
        "order_hint_coreq=dual_linf_under_wire_proven; "
        "distinct_from_dual_linf_criteria_contract=true "
        "(criteria=when may become proven; online_gate=when gate may close; "
        "probe/bridge/warmstart=classic-form diagnostic L∞; "
        "this prep=how fourth-coreq prep lands without proof); "
        "form_current=" + _CASE1_FORM_CURRENT + "; form_label_change_shipped=false; "
        "path_shipped=false; dual_honest_tf_aware_path_present=false; "
        "wire_shipped=false; bundle_shipped=false; isolation_rewrite_shipped=false; "
        "dual_recovery_path=None today; "
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "prep_is_not_dual_linf_under_wire_proof=true; prep_is_not_criteria_met=true; "
        "prep_is_not_gate_flip=true; prep_is_not_wire=true; "
        "prep_is_not_verdict_gate=true; prep_is_not_feature_flag_enable=true; "
        "this_prep_alone_is_not_proof=true; packaging_alone_is_not_proof=true; "
        "dual_linf_criteria_alone_is_anti_criterion=true; "
        "online_linf_gate_criteria_alone_is_anti_criterion=true; "
        "probe_linf_is_not_proof=true; bridge_linf_is_not_proof=true; "
        "warmstart_linf_is_not_proof=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        "Case 1 duals remain PRIMARY free online λ / SECONDARY recovered blender; "
        "this packaging is not dual_linf proven / not gate flip / not wire / "
        "not feature flag enable / not VERDICT dual gate / not criteria alone / "
        "not diagnostic L∞ as proof."
    )
    return {
        "topic": "tf_offline_case1_dual_linf_fourth_coreq_operational_prep",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "prep_is_not_verdict_gate": "true",
        "prep_is_not_feature_flag_enable": "true",
        "this_prep_alone_is_not_proof": "true",
        "packaging_alone_is_not_proof": "true",
...
def format_tf_offline_case1_wire_fifth_coreq_operational_prep_howto() -> Dict[str, str]:
    """Static offline Case-1 wire fifth-coreq *operational prep* How_to.

    Planner-facing dual-ban packaging twin of TF wire fifth-coreq operational prep:
    prep_present=true; wire_shipped=false; wire_ship_allowed=false;
    prep≠ship dual-ban; dual_recovery_path=None;
    first_blocking still isolation_rewrite_with_wire (wire is fifth coreq);
    distinct from wire-ship design / preflight / prior coreq prep packaging.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_wire_fifth_coreq_operational_prep_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_WIRE_FIFTH_COREQ_PREP_ANTI_CRITERIA)
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    flag = _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME
    one_liner = (
        "Offline Case-1 wire fifth-coreq *operational prep* readiness exists "
        "(static packaging): prep_present=true; wire_fifth_coreq_prep_present=true; "
        "operational_prep_present=true; wire_shipped=false; wire_ship_allowed_today=false; "
        "wire_ship_criteria_met_today=false; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        "dual_linf_proof_allowed_today=false; criteria_met_today=false; "
        "online_linf_gate_under_tf_path=open; gate_flip_allowed_today=false; "
        f"feature_flag_name={flag}; feature_flag_enabled_today=false; "
        f"first_blocking_coreq={first_blocking} (wire is fifth coreq, not first); "
        "is_first_blocking_coreq=false; order_hint_index=4; "
        "order_hint_coreq=wire_shipped; "
        "distinct_from_wire_ship_acceptance_design_contract=true "
        "(design=when may ship; preflight=blockers still true; "
        "prior coreq preps=inventory only; this prep=how fifth-coreq prep lands without ship); "
        "form_current=" + _CASE1_FORM_CURRENT + "; form_label_change_shipped=false; "
        "path_shipped=false; dual_honest_tf_aware_path_present=false; "
        "bundle_shipped=false; isolation_rewrite_shipped=false; "
        "dual_recovery_path=None today; "
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "prep_is_not_wire_shipped=true; prep_is_not_wire_ship_allow=true; "
        "prep_is_not_wire_criteria_met=true; prep_is_not_dual_linf_proof=true; "
        "prep_is_not_gate_flip=true; prep_is_not_verdict_gate=true; "
        "prep_is_not_feature_flag_enable=true; "
        "this_prep_alone_is_not_wire_shipped=true; packaging_alone_is_not_wire_shipped=true; "
        "wire_design_alone_is_anti_criterion=true; preflight_alone_is_anti_criterion=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        "Case 1 duals remain PRIMARY free online λ / SECONDARY recovered blender; "
        "this packaging is not wire shipped / not wire allow / not dual_linf proven / "
        "not gate flip / not feature flag enable / not VERDICT dual gate / not design alone / "
        "not preflight alone."
    )
    return {
        "topic": "tf_offline_case1_wire_fifth_coreq_operational_prep",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "prep_is_not_wire_criteria_met": "true",
        "prep_is_not_dual_linf_under_wire_proof": "true",
        ...
        "prep_is_not_verdict_gate": "true",
        "prep_is_not_feature_flag_enable": "true",
        "this_prep_alone_is_not_wire_shipped": "true",
        "packaging_alone_is_not_wire_shipped": "true",
        "packaging_is_not_wire_shipped": "true",
        "packaging_is_not_dual_linf_proven": "true",
        "distinct_from_wire_ship_acceptance_design_contract": "true",
        "distinct_from_offline_wire_preflight": "true",
        "distinct_from_bundle_design_criteria": "true",
        "distinct_from_prior_coreq_operational_preps": "true",
        "this_prep_formalizes_how_fifth_coreq_prep_lands_without_ship": "true",
        "wire_is_fifth_coreq_not_first_blocking": "true",
        "wire_design_alone_is_anti_criterion": "true",
        "preflight_alone_is_anti_criterion": "true",
        "order_hint_is_not_executor": "true",
        "no_auto_wire": "true",
        "units_affine_unchanged": _OFFLINE_TF_UNITS,
        "dual_linf_under_wire_status": _CASE1_DUAL_LINF_UNDER_WIRE_STATUS,
...
def format_tf_offline_case1_isolation_rewrite_first_blocker_execution_scaffold_howto() -> Dict[str, str]:
    ...
    Planner-facing dual-ban packaging twin of TF isolation rewrite execution scaffold:
    scaffold_present=true; isolation_rewrite_shipped=false; isolation_ship_allowed=false;
    scaffold≠ship dual-ban; dual_recovery_path=None;
    first_blocking still isolation_rewrite_with_wire; rewrite-not-delete inventory only.
    Distinct from iso design / ship-met / operational prep / path scaffold / wire fifth prep.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_isolation_rewrite_first_blocker_execution_scaffold_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_ISOLATION_REWRITE_EXECUTION_SCAFFOLD_ANTI_CRITERIA)
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    flag = _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME
    one_liner = (
        "Offline Case-1 isolation rewrite first-blocker *execution scaffold* readiness exists "
        "(static packaging): scaffold_present=true; execution_scaffold_present=true; "
        "isolation_rewrite_scaffold_present=true; isolation_rewrite_shipped=false; "
        "isolation_ship_allowed_today=false; isolation_tests_rewritten_with_wire=false; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        "dual_linf_proof_allowed_today=false; criteria_met_today=false; "
        "online_linf_gate_under_tf_path=open; gate_flip_allowed_today=false; "
        f"feature_flag_name={flag}; feature_flag_enabled_today=false; "
        f"first_blocking_coreq={first_blocking}; is_first_blocking_coreq=true; "
        "order_hint_index=0; order_hint_coreq=isolation_rewrite_with_wire; "
        "rewrite_not_delete=true; suite=tests/test_tf_import_isolation.py; "
        "mutation inventory only (not applied); "
        "distinct_from_isolation_design=true; distinct_from_isolation_ship_met=true; "
        "distinct_from_isolation_operational_prep=true; "
        "distinct_from_path_execution_scaffold=true; distinct_from_wire_fifth_prep=true; "
        "form_current=" + _CASE1_FORM_CURRENT + "; form_label_change_shipped=false; "
        "path_shipped=false; dual_honest_tf_aware_path_present=false; "
        "bundle_shipped=false; wire_shipped=false; dual_recovery_path=None today; "
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "scaffold_is_not_isolation_rewrite_shipped=true; "
        "scaffold_is_not_isolation_ship_allow=true; scaffold_is_not_wire_shipped=true; "
        "scaffold_is_not_dual_linf_under_wire_proof=true; "
        "scaffold_is_not_gate_flip=true; scaffold_is_not_verdict_gate=true; "
        "this_scaffold_alone_is_not_ship_criterion=true; "
        "packaging_alone_is_not_isolation_rewrite_shipped=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        "this packaging is not isolation rewrite shipped / not isolation allow / "
        "not wire shipped / not dual_linf proven / not gate flip / not VERDICT."
    )
    return {
        "topic": "tf_offline_case1_isolation_rewrite_first_blocker_execution_scaffold",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "first_blocking_coreq": first_blocking,
        "is_first_blocking_coreq": "true",
        "order_hint_index": "0",
        "order_hint_coreq": "isolation_rewrite_with_wire",
        "path_shipped": "false",
        "dual_honest_tf_aware_path_present": "false",
        "ship_met_allowed_today": "false",
        "criteria_met_today": "false",
        ...
        "gate_flip_allowed_today": "false",
        "rewrite_not_delete": "true",
        "suite_path": "tests/test_tf_import_isolation.py",
        "mutation_inventory_not_applied": "true",
        "scaffold_is_not_isolation_rewrite_shipped": "true",
        "scaffold_is_not_isolation_ship_allow": "true",
        ...
        "packaging_alone_is_not_isolation_rewrite_shipped": "true",
        "packaging_is_not_dual_linf_proven": "true",
        "distinct_from_isolation_design": "true",
        "distinct_from_isolation_ship_met": "true",
        "distinct_from_isolation_operational_prep": "true",
        "distinct_from_path_execution_scaffold": "true",
...
def format_tf_offline_case1_dual_linf_fourth_coreq_execution_scaffold_howto() -> Dict[str, str]:
    """Static offline Case-1 dual_linf fourth-coreq *execution scaffold* How_to.

    Planner-facing dual-ban packaging twin of TF dual_linf execution scaffold:
    scaffold_present=true; dual_linf_under_wire=unproven; dual_linf_proof_allowed=false;
    criteria_met_today=false; online_linf_gate open; gate_flip_allowed=false;
    proof_composition_path_executed=false; scaffold≠proof dual-ban;
    dual_recovery_path=None; first_blocking still isolation_rewrite_with_wire
    (dual_linf is fourth coreq). Distinct from dual_linf criteria / dual_linf prep /
    online_linf_gate criteria / form scaffold / isolation scaffold / path scaffold.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_dual_linf_fourth_coreq_execution_scaffold_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_ANTI_CRITERIA)
    path_name = _CASE1_DUAL_LINF_PROOF_COMPOSITION_PATH_NAME
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    flag = _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME
    one_liner = (
        "Offline Case-1 dual_linf fourth-coreq *execution scaffold* readiness exists "
        "(static packaging): scaffold_present=true; execution_scaffold_present=true; "
        ...
        f"proof_composition_path_name={path_name}; "
        f"feature_flag_name={flag}; feature_flag_enabled_today=false; "
        f"first_blocking_coreq={first_blocking} (dual_linf is fourth coreq, not first); "
        "is_first_blocking_coreq=false; order_hint_index=3; "
        "order_hint_coreq=dual_linf_under_wire_proven; "
        "proof-composition inventory only (pieces not executed); "
        "distinct_from_dual_linf_under_wire_criteria_contract=true; "
        "distinct_from_dual_linf_fourth_coreq_operational_prep=true; "
        "distinct_from_online_linf_gate_criteria_contract=true; "
        "distinct_from_form_label_execution_scaffold=true; "
        "distinct_from_isolation_execution_scaffold=true; "
        "distinct_from_path_execution_scaffold=true; "
        "path_shipped=false; wire_shipped=false; bundle_shipped=false; "
        "isolation_rewrite_shipped=false; form_label_change_shipped=false; "
        "form=classic_2block_excel_path; dual_recovery_path=None today; "
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "scaffold_is_not_dual_linf_proven=true; "
        "scaffold_is_not_dual_linf_proof_allow=true; "
        "scaffold_is_not_gate_flip=true; scaffold_is_not_wire=true; "
        "scaffold_is_not_verdict_gate=true; "
        "this_scaffold_alone_is_not_dual_linf_proof=true; "
        "packaging_alone_is_not_dual_linf_proven=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        "this packaging is not dual_linf proven / not proof allow / "
        "not gate flip / not wire shipped / not form flip / not VERDICT."
    )
    return {
        "topic": "tf_offline_case1_dual_linf_fourth_coreq_execution_scaffold",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "online_linf_gate_under_tf_path": "open",
        "gate_flip_allowed_today": "false",
        "proof_composition_path_name": path_name,
        "proof_composition_path_executed_today": "false",
        "proof_composition_inventory_not_executed": "true",
        "isolation_rewrite_shipped": "false",
        "isolation_tests_rewritten_with_wire": "false",
        "form_label_change_shipped": "false",
        "wire_shipped": "false",
...
def format_tf_offline_case1_wire_fifth_coreq_execution_scaffold_howto() -> Dict[str, str]:
    """Static offline Case-1 wire fifth-coreq *execution scaffold* How_to.

    Planner-facing dual-ban packaging twin of TF wire execution scaffold:
    scaffold_present=true; wire_shipped=false; wire_ship_allowed=false;
    wire_land_path_executed=false; dual_linf_under_wire=unproven;
    dual_linf_proof_allowed=false; criteria_met_today=false; online_linf_gate open;
    gate_flip_allowed=false; scaffold≠wire dual-ban; dual_recovery_path=None;
    first_blocking still isolation_rewrite_with_wire (wire is fifth coreq).
    Distinct from wire fifth prep / wire-ship design / preflight / dual_linf scaffold /
    form scaffold / isolation scaffold / path scaffold.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_wire_fifth_coreq_execution_scaffold_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_WIRE_FIFTH_COREQ_EXECUTION_SCAFFOLD_ANTI_CRITERIA)
    path_name = _CASE1_WIRE_LAND_PATH_NAME
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    flag = _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME
    one_liner = (
        "Offline Case-1 wire fifth-coreq *execution scaffold* readiness exists "
        "(static packaging): scaffold_present=true; execution_scaffold_present=true; "
        "wire_scaffold_present=true; wire_fifth_coreq_scaffold_present=true; "
        "wire_shipped=false; wire_ship_allowed_today=false; "
        "wire_ship_criteria_met_today=false; wire_land_path_executed_today=false; "
        f"wire_land_path_name={path_name}; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        "dual_linf_proof_allowed_today=false; criteria_met_today=false; "
        "online_linf_gate_under_tf_path=open; gate_flip_allowed_today=false; "
        f"feature_flag_name={flag}; feature_flag_enabled_today=false; "
        ...
        "is_first_blocking_coreq=false; order_hint_index=4; "
        "order_hint_coreq=wire_shipped; "
        "wire-land composition inventory only (pieces not executed); "
        "distinct_from_wire_fifth_operational_prep=true; "
        "distinct_from_wire_ship_acceptance_design=true; "
        "distinct_from_offline_wire_preflight=true; "
        "distinct_from_dual_linf_fourth_coreq_execution_scaffold=true; "
        "distinct_from_form_label_execution_scaffold=true; "
        "distinct_from_isolation_execution_scaffold=true; "
        "distinct_from_path_execution_scaffold=true; "
        "path_shipped=false; bundle_shipped=false; "
        "isolation_rewrite_shipped=false; form_label_change_shipped=false; "
        "form=classic_2block_excel_path; dual_recovery_path=None today; "
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "scaffold_is_not_wire_shipped=true; scaffold_is_not_wire_allow=true; "
        "scaffold_is_not_dual_linf_proven=true; "
        "scaffold_is_not_gate_flip=true; scaffold_is_not_verdict_gate=true; "
        "this_scaffold_alone_is_not_wire_ship=true; "
        "packaging_alone_is_not_wire_shipped=true; "
        "order_hint_is_not_executor=true; no_auto_wire=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        "this packaging is not wire shipped / not wire allow / "
        "not dual_linf proven / not gate flip / not form flip / not VERDICT."
    )
    return {
        "topic": "tf_offline_case1_wire_fifth_coreq_execution_scaffold",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "online_linf_gate_under_tf_path": "open",
        "gate_flip_allowed_today": "false",
        "wire_land_path_name": path_name,
        "wire_land_path_executed_today": "false",
        "wire_land_composition_inventory_not_executed": "true",
        "isolation_rewrite_shipped": "false",
        "isolation_tests_rewritten_with_wire": "false",
        "form_label_change_shipped": "false",
        "wire_shipped": "false",
        ...
        "feature_flag_enabled_today": "false",
        "scaffold_is_not_wire_shipped": "true",
        "scaffold_is_not_wire_allow": "true",
        "scaffold_is_not_dual_linf_proven": "true",
        "scaffold_is_not_gate_flip": "true",
        "scaffold_is_not_wire": "true",
        "scaffold_is_not_verdict_gate": "true",
...
_CASE1_BUNDLE_EXECUTION_SCAFFOLD_KIND = (
    "offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold"
)
_CASE1_BUNDLE_LAND_PATH_NAME = (
    "compose_bundle_design_criteria_order_hint_preps_scaffolds_flag_recovery_checklist_when_multi_blocker_ship"
)
_CASE1_BUNDLE_EXECUTION_SCAFFOLD_ANTI_CRITERIA = (
    "this_scaffold_alone",
...
_OFFLINE_TF_CASE1_BUNDLE_EXECUTION_SCAFFOLD_NOTE = (
    "bundle companion execution scaffold packaged (scaffold_present; "
    "bundle_shipped=false; bundle_ship_allowed=false; land not executed; "
    "companion not order_hint primary; first_blocking still isolation; "
    "scaffold≠bundle; dual_recovery_path=None; dual-ban; not VERDICT; "
    "not gate flip; not form flip; not wire shipped; not dual_linf proven)"
)


def format_tf_offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_howto() -> Dict[str, str]:
    """Static offline Case-1 dual-honest multi-blocker wire *bundle companion execution scaffold* How_to.

    Planner-facing dual-ban packaging twin of TF bundle companion execution scaffold:
    scaffold_present=true; bundle_shipped=false; bundle_ship_allowed=false;
    bundle_land_path_executed=false; scaffold≠bundle dual-ban; dual_recovery_path=None;
    first_blocking still isolation_rewrite_with_wire (companion not order_hint primary);
    is_first_blocking_coreq=false; distinct from bundle design / ship-met criteria /
    bundle operational prep / wire fifth scaffold packaging.
    Does **not** load tf_linear_blocks or tensorflow; does **not** call
    offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_report. No Index growth.
    """
    open_ids = ",".join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)
    anti = ",".join(_CASE1_BUNDLE_EXECUTION_SCAFFOLD_ANTI_CRITERIA)
    path_name = _CASE1_BUNDLE_LAND_PATH_NAME
    first_blocking = _CASE1_FIRST_BLOCKING_COREQ
    flag = _CASE1_PATH_DESIGN_FEATURE_FLAG_NAME
    one_liner = (
        "Offline Case-1 dual-honest multi-blocker wire *bundle companion execution scaffold* "
        "readiness exists (static packaging): scaffold_present=true; execution_scaffold_present=true; "
        "bundle_scaffold_present=true; companion_bundle_scaffold_present=true; "
        "bundle_shipped=false; bundle_ship_allowed_today=false; criteria_met_today=false; "
        "bundle_land_path_executed_today=false; "
        f"bundle_land_path_name={path_name}; "
        "wire_shipped=false; wire_ship_allowed_today=false; "
        f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
        "dual_linf_proof_allowed_today=false; online_linf_gate_under_tf_path=open; "
        "gate_flip_allowed_today=false; "
        ...
        "is_first_blocking_coreq=false; companion_not_order_hint_primary=true; "
        "order_hint_is_not_executor=true; "
        "how-bundle-land composition inventory only (pieces not executed); "
        "distinct_from_bundle_design_contract=true; "
        "distinct_from_bundle_ship_met_criteria_contract=true; "
        "distinct_from_bundle_operational_prep=true; "
        "distinct_from_wire_fifth_coreq_execution_scaffold=true; "
        "distinct_from_wire_fifth_coreq_operational_prep=true; "
        "form_current=" + _CASE1_FORM_CURRENT + "; form_label_change_shipped=false; "
        "path_shipped=false; dual_honest_tf_aware_path_present=false; "
        ...
        f"dual_recovery_path_planned_when_shipped="
        f"{_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED} (not pure-ADMM); "
        "scaffold_is_not_bundle_shipped=true; scaffold_is_not_bundle_allow=true; "
        "scaffold_is_not_bundle_land_executed=true; scaffold_is_not_wire_shipped=true; "
        "scaffold_is_not_dual_linf_proven=true; scaffold_is_not_gate_flip=true; "
        "scaffold_is_not_verdict_gate=true; this_scaffold_alone_is_not_bundle_ship=true; "
        "packaging_alone_is_not_bundle_shipped=true; "
        f"open checklist ({open_ids}); "
        f"units_affine_unchanged={_OFFLINE_TF_UNITS} (no silent BLENDER); "
        f"anti_criteria_today={anti}; does not clear wire_blockers; "
        ...
        "this packaging is not bundle shipped / not bundle allow / not land executed / "
        "not wire shipped / not dual_linf proven / not gate flip / not VERDICT dual gate / "
        "not design alone / not criteria alone / not prep alone / not wire scaffold alone."
    )
    return {
        "topic": "tf_offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold",
        "units": "CDU+Blender",
        "on_case1_solve": "false",
        "not_case1_solve": "true",
        "form_current": _CASE1_FORM_CURRENT,
        ...
        "bundle_shipped": "false",
        "bundle_ship_allowed_today": "false",
        "bundle_land_path_name": path_name,
        "bundle_land_path_executed_today": "false",
        "wire_ship_allowed_today": "false",
        "isolation_ship_allowed_today": "false",
        "online_linf_gate_under_tf_path": "open",
        "gate_flip_allowed_today": "false",
        "scaffold_is_not_bundle_shipped": "true",
        "scaffold_is_not_bundle_allow": "true",
        "scaffold_is_not_bundle_land_executed": "true",
        "scaffold_is_not_wire_shipped": "true",
        "scaffold_is_not_wire_allow": "true",
        "scaffold_is_not_dual_linf_proven": "true",
        "scaffold_is_not_gate_flip": "true",
        "scaffold_is_not_verdict_gate": "true",
        "this_scaffold_alone_is_not_bundle_ship": "true",
        "packaging_alone_is_not_bundle_shipped": "true",
        "packaging_is_not_bundle_shipped": "true",
        "packaging_is_not_dual_linf_proven": "true",
        "distinct_from_bundle_design_contract": "true",
        ...
        "distinct_from_wire_fifth_coreq_operational_prep": "true",
        "distinct_from_wire_fifth_coreq_execution_scaffold": "true",
        "this_scaffold_formalizes_how_bundle_land_composition_without_ship": "true",
        "order_hint_is_not_executor": "true",
        "no_auto_wire": "true",
        "no_auto_bundle_ship": "true",
        "units_affine_unchanged": _OFFLINE_TF_UNITS,
...
def format_tf_offline_ladder_toc_howto() -> Dict[str, str]:
    ...
    ladder: List[tuple[str, str]] = [
        ...
        (
            ...
            "dual_linf flip criteria (status unproven; ≠ online_gate criteria)",
        ),
        (
            "tf_offline_case1_form_label_second_coreq_operational_prep",
            "form_label second-coreq operational prep (prep≠ship; form classic)",
        ),
        (
            "tf_offline_case1_path_third_coreq_operational_prep",
            "path third-coreq operational prep (prep≠path ship; 3rd=path)",
        ),
        (
            "tf_offline_case1_dual_linf_fourth_coreq_operational_prep",
            "dual_linf fourth-coreq operational prep (prep≠proof; 4th=dual_linf)",
        ),
        (
            "tf_offline_case1_wire_fifth_coreq_operational_prep",
            "wire fifth-coreq operational prep (prep≠wire ship; 5th=wire)",
        ),
        (
        ...
        (
            "tf_offline_case1_dual_linf_fourth_coreq_execution_scaffold",
            "dual_linf fourth-coreq execution scaffold (scaffold≠proof; dual_linf unproven; gate open; composition inventory only)",
        ),
        (
        ...
        (
            "tf_offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold",
            "bundle companion execution scaffold (scaffold≠bundle shipped; allow=false; land not executed; companion≠order_hint primary; 1st=iso)",
        ),
    ]
    dual_ban = (
    ...
    return {
        ...
        "includes_dual_linf_under_wire_criteria_contract": "true",
        "includes_form_label_second_coreq_operational_prep": "true",
        "includes_path_third_coreq_operational_prep": "true",
        ...
        "includes_bundle_companion_execution_scaffold": "true",
...
# Excel-local mirrors of dual-honest multi-blocker wire *implementation blueprint* packaging (#64 twin).
# Static only — never import tf_linear_blocks. blueprint_present / implementation_blueprint_present
# / wire_go_board_present = packaging existence; first_blocking_coreq static expected label;
# rehearsal+scaffold linked; all ship flags stay False. Packaging alone / this blueprint alone /
# first_blocking alone / go-board alone are never path/wire/bundle/isolation/form ship enablers.
# Distinct from multi-blocker design (#57), ship-met criteria (#59), scaffold packaging (#61),
# and rehearsal packaging (#63); TF live blueprint is separate.
_CASE1_BLUEPRINT_PRESENT = True
_CASE1_IMPLEMENTATION_BLUEPRINT_PRESENT = True
_CASE1_WIRE_GO_BOARD_PRESENT = True
_CASE1_FIRST_BLOCKING_COREQ = "isolation_rewrite_with_wire"
_CASE1_BLUEPRINT_KIND = "offline_case1_dual_honest_multi_blocker_wire_implementation_blueprint"
# Excel-local mirror of CASE1_DUAL_HONEST_MULTI_BLOCKER_WIRE_IMPLEMENTATION_BLUEPRINT_ANTI_CRITERIA_TODAY (#64).
...
_CASE1_FORM_LABEL_SECOND_COREQ_PREP_KIND = (
    "offline_case1_form_label_second_coreq_operational_prep"
)
_CASE1_FORM_LABEL_MUTATION_PATH_NAME = (
    "feature_flag_enable_tf_affine_case1_wire_then_set_model_form_to_planned"
)
_CASE1_FORM_LABEL_SECOND_COREQ_PREP_ANTI_CRITERIA = (
    "this_prep_alone",
    "form_label_criteria_alone",
    "form_label_change_shipped_criteria_contract_alone",
    "form_registration_alone",
...
_CASE1_PATH_THIRD_COREQ_PREP_KIND = (
    "offline_case1_path_third_coreq_operational_prep"
)
_CASE1_PATH_THIRD_COREQ_ORDER_HINT_INDEX = "2"
_CASE1_PATH_THIRD_COREQ_ORDER_HINT_COREQ = "dual_honest_tf_aware_path_present"
_CASE1_PATH_THIRD_COREQ_PREP_ANTI_CRITERIA = (
    "this_prep_alone",
    "path_design_alone",
    "path_present_criteria_alone",
    "scaffold_alone",
    "execution_scaffold_alone",
    "rehearsal_alone",
    "blueprint_alone",
    "packaging_alone",
    "this_prep_packaging_alone",
...
_OFFLINE_TF_CASE1_PATH_THIRD_COREQ_OPERATIONAL_PREP_NOTE = (
    "path third-coreq operational prep packaged (prep_present; path_shipped=false; "
    "dual_honest_tf_aware_path_present ship-met=false; feature_flag=false; "
    "first_blocking still isolation; prep≠ship; dual_recovery_path=None; "
    "distinct from design/criteria/scaffold; dual-ban; not VERDICT; not wire)"
)

_CASE1_DUAL_LINF_FOURTH_COREQ_PREP_KIND = (
    "offline_case1_dual_linf_fourth_coreq_operational_prep"
)
_CASE1_DUAL_LINF_FOURTH_COREQ_ORDER_HINT_INDEX = "3"
_CASE1_DUAL_LINF_FOURTH_COREQ_ORDER_HINT_COREQ = "dual_linf_under_wire_proven"
...
_OFFLINE_TF_CASE1_DUAL_LINF_FOURTH_COREQ_OPERATIONAL_PREP_NOTE = (
    "dual_linf fourth-coreq operational prep packaged (prep_present; "
    "dual_linf=unproven; proof_allowed=false; gate=open; first_blocking still isolation; "
    "prep≠proof; dual_recovery_path=None; distinct from criteria/probe; dual-ban; "
    "not VERDICT; not gate flip; not wire)"
)


_CASE1_WIRE_FIFTH_COREQ_PREP_KIND = (
    "offline_case1_wire_fifth_coreq_operational_prep"
)
_CASE1_WIRE_FIFTH_COREQ_ORDER_HINT_INDEX = "4"
_CASE1_WIRE_FIFTH_COREQ_ORDER_HINT_COREQ = "wire_shipped"
...
_OFFLINE_TF_CASE1_WIRE_FIFTH_COREQ_OPERATIONAL_PREP_NOTE = (
    "wire fifth-coreq operational prep packaged (prep_present; wire_shipped=false; "
...
_CASE1_ISOLATION_REWRITE_EXECUTION_SCAFFOLD_ORDER_HINT_INDEX = "0"
_CASE1_ISOLATION_REWRITE_EXECUTION_SCAFFOLD_ORDER_HINT_COREQ = (
    "isolation_rewrite_with_wire"
)
_CASE1_ISOLATION_REWRITE_EXECUTION_SCAFFOLD_ANTI_CRITERIA = (
    "this_scaffold_alone",
    "this_execution_scaffold_alone",
    "design_alone",
    "ship_met_criteria_alone",
...
_OFFLINE_TF_CASE1_ISOLATION_REWRITE_FIRST_BLOCKER_EXECUTION_SCAFFOLD_NOTE = (
    "isolation rewrite first-blocker execution scaffold packaged (scaffold_present; "
...
_CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_KIND = (
    "offline_case1_dual_linf_fourth_coreq_execution_scaffold"
)
_CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_ORDER_HINT_INDEX = "3"
_CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_ORDER_HINT_COREQ = (
    "dual_linf_under_wire_proven"
)
_CASE1_DUAL_LINF_PROOF_COMPOSITION_PATH_NAME = (
    "compose_dual_linf_criteria_gate_diagnostics_planned_recovery_checklist_"
    "under_tf_aware_form_when_shipped"
)
_CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_ANTI_CRITERIA = (
    "this_scaffold_alone",
    "this_execution_scaffold_alone",
    "design_alone",
    "ship_met_criteria_alone",
...
_OFFLINE_TF_CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_NOTE = (
    "dual_linf fourth-coreq execution scaffold packaged (scaffold_present; "
    "dual_linf unproven; dual_linf_proof_allowed=false; gate open; "
    "gate_flip_allowed=false; proof composition not executed; first_blocking still isolation; "
    "scaffold≠proof; dual_recovery_path=None; dual-ban; not VERDICT; not gate flip; "
    "not form flip; not wire; not dual_linf proven)"
)


_CASE1_WIRE_FIFTH_COREQ_EXECUTION_SCAFFOLD_KIND = (
    "offline_case1_wire_fifth_coreq_execution_scaffold"
)
_CASE1_WIRE_FIFTH_COREQ_EXECUTION_SCAFFOLD_ORDER_HINT_INDEX = "4"
_CASE1_WIRE_FIFTH_COREQ_EXECUTION_SCAFFOLD_ORDER_HINT_COREQ = "wire_shipped"
_CASE1_WIRE_LAND_PATH_NAME = (
    "compose_wire_design_prep_preflight_prior_scaffolds_flag_recovery_"
    "checklist_when_multi_blocker_ship"
)
_CASE1_WIRE_FIFTH_COREQ_EXECUTION_SCAFFOLD_ANTI_CRITERIA = (
    "this_scaffold_alone",
    "this_execution_scaffold_alone",
    "design_alone",
    "ship_met_criteria_alone",
...
_OFFLINE_TF_CASE1_WIRE_FIFTH_COREQ_EXECUTION_SCAFFOLD_NOTE = (
    "wire fifth-coreq execution scaffold packaged (scaffold_present; "
    "wire_shipped=false; wire_ship_allowed=false; wire_land not executed; "
    "dual_linf unproven; dual_linf_proof_allowed=false; gate open; "
    "gate_flip_allowed=false; first_blocking still isolation; "
    "scaffold≠wire; dual_recovery_path=None; dual-ban; not VERDICT; not gate flip; "
    "not form flip; not wire shipped; not dual_linf proven)"
)



# Excel-local mirror of CASE1_DUAL_LINF_UNDER_WIRE_FLIP_CRITERIA keys (#cycle dual_linf criteria).
...
_CASE1_DUAL_LINF_UNDER_WIRE_CRITERIA_KIND = (
...
_OFFLINE_TF_INDEX_WHAT = (
    ...
    "scaffold+rehearse+bp (pres; 1st=iso; ship=false) — "
...
_OFFLINE_TF_CASE1_DUAL_HONEST_MULTI_BLOCKER_WIRE_IMPLEMENTATION_BLUEPRINT_NOTE = (
    "offline Case-1 dual-honest multi-blocker wire *implementation blueprint / go-board* readiness — "
    "blueprint_present=true; implementation_blueprint_present=true; wire_go_board_present=true; "
    f"first_blocking_coreq={_CASE1_FIRST_BLOCKING_COREQ}; "
    "rehearsal_present=true; wire_rehearsal_present=true; "
    "scaffold_present=true; execution_scaffold_present=true; "
    "path_shipped=false; dual_honest_tf_aware_path_present ship-met=false; "
    ...
    "form_label_ship_allowed_today=false; path_design_present=true; "
    "wire_ship_allowed_today=false; "
    "go-board / order_hint / prep-map static labels only (not executor; first_blocking alone ≠ ship); "
    "order_hint_is_not_executor=true; no_auto_wire=true; dual-ban; "
    "distinct from scaffold / rehearsal / bundle design / ship-met criteria packaging; "
    f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
    f"open_checklist={','.join(_CASE1_DUAL_LINF_PROOF_CHECKLIST_OPEN_IDS)}; "
    "online_linf_gate_under_tf_path=open; gate_flip_allowed_today=false; "
    "no_blender_offline_affine_kernel still true; "
    ...
    f"dual_recovery_path_planned={_CASE1_PATH_DESIGN_DUAL_RECOVERY_PLANNED}; "
    "does not clear wire_blockers; not form flip; not dual L∞ proven under wire; "
    "packaging ≠ blueprint_present-as-ship ≠ path_shipped ≠ ship-met "
    "≠ wire_shipped ≠ bundle_shipped ≠ isolation_rewrite_shipped "
    "≠ form_label_change_shipped ≠ form flip ≠ VERDICT; "
...
_OFFLINE_TF_CASE1_DUAL_LINF_UNDER_WIRE_CRITERIA_CONTRACT_NOTE = (
    "dual_linf flip-criteria contract readiness (criteria_present; dual_linf=unproven; "
    "criteria_met_today=false; dual_linf_proof_allowed_today=false; distinct from "
    "online_linf_gate criteria; dual-ban; not VERDICT; not wire)"
)

_OFFLINE_TF_CASE1_FORM_LABEL_SECOND_COREQ_OPERATIONAL_PREP_NOTE = (
    "form_label second-coreq operational prep packaging present (prep_present=true; "
    "form=classic; form_label_change_shipped=false; form_label_ship_allowed=false; "
    "mutation not executed; first_blocking=isolation; prep≠ship; dual_recovery_path=None; "
    "not VERDICT; not form flip; not wire)"
)
...
_OFFLINE_TF_READINESS_NOTE = (
    ...
    "dual-honest multi-blocker wire implementation blueprint / go-board readiness "
    "(blueprint_present; first_blocking=isolation_rewrite_with_wire; rehearsal+scaffold linked; "
    "path/wire/bundle ship=false; go-board static; dual-ban; not VERDICT) + "
    "form_label second-coreq operational prep readiness "
    "(prep_present; form classic; form_label_change_shipped=false; mutation not executed; "
    "first_blocking still isolation; prep≠ship; dual-ban; not VERDICT) — "
    ...
    "blueprint packaging ≠ path shipped / ≠ wire_shipped / ≠ bundle shipped / ≠ isolation rewrite shipped / "
    "≠ form ship / ≠ ship-met / ≠ first_blocking_as_ship / ≠ VERDICT / ≠ dual L∞ under wire proof; "
    "form second-coreq prep packaging ≠ form_label_change_shipped / ≠ form flip / ≠ path shipped / "
    "≠ wire_shipped / ≠ isolation rewrite shipped / ≠ VERDICT / ≠ dual L∞ under wire proof; "
    "live_lambda_source must be labeled; "
...
def format_planner_honesty_package(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    + dual-honest multi-blocker wire implementation blueprint packaging
    ...
    tf_dual_linf_scaffold = (
        format_tf_offline_case1_dual_linf_fourth_coreq_execution_scaffold_howto()
    )
    tf_wire_scaffold = (
        format_tf_offline_case1_wire_fifth_coreq_execution_scaffold_howto()
    )
    tf_bundle_scaffold = (
        format_tf_offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_howto()
    )
    ...
    meta = {
        ...
        "offline_tf_case1_dual_linf_under_wire_criteria_contract_ready": True,  # static; not live dual_linf criteria report
        "offline_tf_dual_linf_criteria_present": True,  # static packaging existence only
        "offline_tf_case1_form_label_second_coreq_operational_prep_ready": True,  # static; not live form prep report
        "offline_tf_form_label_second_coreq_prep_present": True,  # static packaging existence only
        "offline_tf_case1_path_third_coreq_operational_prep_ready": True,  # static; not live path prep report
        "offline_tf_path_third_coreq_prep_present": True,  # static packaging existence only
        "offline_tf_case1_dual_linf_fourth_coreq_operational_prep_ready": True,  # static; not live dual_linf prep report
        ...
        "offline_tf_case1_dual_linf_fourth_coreq_execution_scaffold_ready": True,  # static; not live dual_linf scaffold report
        "offline_tf_case1_wire_fifth_coreq_execution_scaffold_ready": True,  # static; not live wire scaffold report
        "offline_tf_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_ready": True,  # static; not live bundle scaffold
        "offline_tf_bundle_scaffold_present": True,  # static packaging existence only
        "offline_tf_companion_bundle_scaffold_present": True,  # static packaging existence only
        "offline_tf_dual_linf_scaffold_present": True,  # static packaging existence only
        ...
        "offline_tf_first_blocking_coreq": _CASE1_FIRST_BLOCKING_COREQ,  # static expected label
        ...
        "offline_tf_case1_dual_linf_under_wire_criteria_contract": (
            _OFFLINE_TF_CASE1_DUAL_LINF_UNDER_WIRE_CRITERIA_CONTRACT_NOTE
        ),
        "offline_tf_case1_form_label_second_coreq_operational_prep": (
            _OFFLINE_TF_CASE1_FORM_LABEL_SECOND_COREQ_OPERATIONAL_PREP_NOTE
        ),
        "offline_tf_case1_path_third_coreq_operational_prep": (
            _OFFLINE_TF_CASE1_PATH_THIRD_COREQ_OPERATIONAL_PREP_NOTE
        ),
        "offline_tf_case1_dual_linf_fourth_coreq_operational_prep": (
            _OFFLINE_TF_CASE1_DUAL_LINF_FOURTH_COREQ_OPERATIONAL_PREP_NOTE
        ),
        ...
        "offline_tf_case1_dual_linf_fourth_coreq_execution_scaffold": (
            _OFFLINE_TF_CASE1_DUAL_LINF_FOURTH_COREQ_EXECUTION_SCAFFOLD_NOTE
        ),
        "offline_tf_case1_wire_fifth_coreq_execution_scaffold": (
            _OFFLINE_TF_CASE1_WIRE_FIFTH_COREQ_EXECUTION_SCAFFOLD_NOTE
        ),
        "offline_tf_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold": (
            _OFFLINE_TF_CASE1_BUNDLE_EXECUTION_SCAFFOLD_NOTE
        ),
        ...
        "planner_one_liner": (
            ...
            f"isolation rewrite first-blocker execution scaffold readiness "
            f"(scaffold_present; isolation_rewrite_shipped=false; isolation_ship_allowed=false; "
            f"inventory only; first_blocking={_CASE1_FIRST_BLOCKING_COREQ}; scaffold≠ship; dual-ban; not VERDICT) + "
            f"dual_linf_under_wire flip-criteria contract readiness "
            f"(criteria_present; dual_linf=unproven; met=false; proof_allowed=false; "
            f"distinct from online_linf_gate criteria; dual-ban; not VERDICT) "
    ...
    summary_pairs = [
        ...
        ("offline_tf_blueprint_present", True),
        ("offline_tf_implementation_blueprint_present", True),
        ...
        ("offline_tf_dual_linf_criteria_present", True),
        ("offline_tf_case1_dual_linf_under_wire_criteria_contract_ready", True),
        ("offline_tf_dual_linf_under_wire_status", "unproven"),
        ("offline_tf_dual_linf_proof_allowed_today", False),
        ("offline_tf_dual_linf_criteria_met_today", False),
        (
            "offline_tf_case1_form_label_second_coreq_operational_prep",
            _OFFLINE_TF_CASE1_FORM_LABEL_SECOND_COREQ_OPERATIONAL_PREP_NOTE,
        ),
        ("offline_tf_form_label_second_coreq_prep_present", True),
        ("offline_tf_case1_form_label_second_coreq_operational_prep_ready", True),
        (
            "offline_tf_case1_path_third_coreq_operational_prep",
            _OFFLINE_TF_CASE1_PATH_THIRD_COREQ_OPERATIONAL_PREP_NOTE,
        ),
        ("offline_tf_path_third_coreq_prep_present", True),
        ("offline_tf_case1_path_third_coreq_operational_prep_ready", True),
        (
            "offline_tf_case1_dual_linf_fourth_coreq_operational_prep",
            _OFFLINE_TF_CASE1_DUAL_LINF_FOURTH_COREQ_OPERATIONAL_PREP_NOTE,
        ),
        ("offline_tf_dual_linf_fourth_coreq_prep_present", True),
        ("offline_tf_case1_dual_linf_fourth_coreq_operational_prep_ready", True),
        (
            "offline_tf_case1_wire_fifth_coreq_operational_prep",
            _OFFLINE_TF_CASE1_WIRE_FIFTH_COREQ_OPERATIONAL_PREP_NOTE,
        ),
        ...
        ("offline_tf_bundle_scaffold_present", True),
        ("offline_tf_companion_bundle_scaffold_present", True),
        ("offline_tf_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_ready", True),
        (
            "offline_tf_case1_isolation_rewrite_first_blocker_execution_scaffold",
            _OFFLINE_TF_CASE1_ISOLATION_REWRITE_FIRST_BLOCKER_EXECUTION_SCAFFOLD_NOTE,
        ),
        ("offline_tf_isolation_rewrite_scaffold_present", True),
        ("offline_tf_case1_isolation_rewrite_first_blocker_execution_scaffold_ready", True),
    ...
    return {
        ...
        "tf_offline_case1_dual_linf_under_wire_criteria_contract": tf_dual_linf_criteria,
        "tf_offline_case1_form_label_second_coreq_operational_prep": tf_form_label_second_coreq_prep,
        ...
        "tf_offline_case1_wire_fifth_coreq_execution_scaffold": tf_wire_scaffold,
        "tf_offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold": tf_bundle_scaffold,
...
def planner_honesty_check_rows(report: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {
            "check": "offline_tf_dual_linf_under_wire_criteria_not_proof",
            "predicted": (
                "dual_linf flip-criteria packaging present; criteria_present=true; "
                "dual_linf_under_wire=unproven; criteria_met_today=false; "
                "dual_linf_proof_allowed_today=false; distinct from online_linf_gate criteria; "
                "criteria≠proven ≠ wire ≠ VERDICT; dual_recovery_path=None on TF surface"
            ),
            "actual": (
                "static honesty — offline_tf_case1_dual_linf_under_wire_criteria_contract_ready=true; "
                "criteria formalized ≠ dual L∞ under wire proof; online_linf_gate stays open; "
                "first_blocking remains isolation_rewrite_with_wire; packaging alone ≠ proof"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        ...
        {
            "check": "offline_tf_path_third_coreq_prep_not_path_ship",
            "predicted": (
                "path third-coreq operational prep packaging present; prep_present=true; "
                "path_shipped=false; dual_honest_tf_aware_path_present=false; "
                "ship_met_allowed_today=false; path_present_criteria_met_today=false; "
                "feature_flag_enabled_today=false; first_blocking still isolation; "
                "prep≠path ship ≠ ship-met ≠ wire ≠ feature flag ≠ VERDICT; dual_recovery_path=None"
            ),
            "actual": (
                "static honesty — offline_tf_case1_path_third_coreq_operational_prep_ready=true; "
                "prep formalized ≠ path_shipped; dual_honest_tf_aware_path_present ship-met remains false; "
                "feature flag named not enabled; packaging alone ≠ path ship; "
                "distinct from path design / present criteria / scaffold packaging"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        {
            "check": "offline_tf_dual_linf_fourth_coreq_prep_not_dual_linf_proof",
            "predicted": (
                "dual_linf fourth-coreq operational prep packaging present; prep_present=true; "
                ...
                "feature_flag_enabled_today=false; first_blocking still isolation; "
                "prep≠dual_linf proven ≠ criteria_met ≠ gate flip ≠ wire ≠ VERDICT; dual_recovery_path=None"
            ),
            "actual": (
                "static honesty — offline_tf_case1_dual_linf_fourth_coreq_operational_prep_ready=true; "
                "prep formalized ≠ dual_linf proven; dual_linf_proof_allowed_today remains false; "
                "gate stays open; packaging alone ≠ dual_linf proof; "
                "distinct from dual_linf criteria / online_linf_gate criteria / probe packaging"
            ),
            "abs_err": 0.0,
            "ok": True,
        ...
        {
            "check": "offline_tf_wire_fifth_coreq_prep_not_wire_shipped",
            "predicted": (
                "wire fifth-coreq operational prep packaging present; prep_present=true; "
                "wire_shipped=false; wire_ship_allowed_today=false; "
                "wire_ship_criteria_met_today=false; dual_linf_under_wire=unproven; "
                "dual_linf_proof_allowed_today=false; online_linf_gate=open; "
                "gate_flip_allowed_today=false; feature_flag_enabled_today=false; "
                "first_blocking still isolation; order_hint_index=4; "
                "prep≠wire shipped ≠ wire allow ≠ dual_linf proven ≠ gate flip ≠ VERDICT; "
                "dual_recovery_path=None"
            ),
            "actual": (
                "static honesty — offline_tf_case1_wire_fifth_coreq_operational_prep_ready=true; "
        ...
        {
            "check": "offline_tf_isolation_rewrite_execution_scaffold_not_rewrite_shipped",
            "predicted": (
                "isolation rewrite first-blocker execution scaffold packaging present; "
                "scaffold_present=true; isolation_rewrite_shipped=false; "
                "isolation_ship_allowed_today=false; isolation_tests_rewritten_with_wire=false; "
                "dual_linf_under_wire=unproven; dual_linf_proof_allowed_today=false; "
                "online_linf_gate=open; gate_flip_allowed_today=false; "
                "feature_flag_enabled_today=false; first_blocking still isolation; "
                "order_hint_index=0; scaffold≠rewrite ship ≠ isolation allow ≠ wire ≠ "
                "dual_linf proven ≠ gate flip ≠ VERDICT; dual_recovery_path=None"
            ),
            "actual": (
                "static honesty — offline_tf_case1_isolation_rewrite_first_blocker_execution_scaffold_ready=true; "
        ...
        {
            "check": "offline_tf_wire_execution_scaffold_not_wire_shipped",
            "predicted": (
                "wire fifth-coreq execution scaffold packaging present; "
                "scaffold_present=true; wire_shipped=false; "
                "wire_ship_allowed_today=false; wire_ship_criteria_met_today=false; "
                "wire_land_path_executed_today=false; dual_linf_under_wire=unproven; "
                "dual_linf_proof_allowed_today=false; criteria_met_today=false; "
                "online_linf_gate=open; gate_flip_allowed_today=false; "
                "form=classic_2block_excel_path; form_label_change_shipped=false; "
                "feature_flag_enabled_today=false; first_blocking still isolation; "
                "order_hint_index=4; is_first_blocking_coreq=false; "
                "scaffold≠wire shipped ≠ wire allow ≠ dual_linf proven ≠ gate flip ≠ "
                "form ship ≠ isolation rewrite ≠ VERDICT; dual_recovery_path=None; "
                "PRIMARY online λ owns dual gate language"
            ),
            "actual": (
                "static honesty — offline_tf_case1_wire_fifth_coreq_execution_scaffold_ready=true; "
                "scaffold formalized ≠ wire shipped; wire_ship_allowed remains false; "
                "wire land path not executed; dual_linf stays unproven; gate stays open; "
                "packaging alone ≠ wire shipped; distinct from wire fifth prep / wire design / "
                "preflight / dual_linf scaffold / form scaffold / isolation scaffold packaging"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        {
            "check": "offline_tf_bundle_execution_scaffold_not_bundle_shipped",
            "predicted": (
                "bundle companion execution scaffold packaging present; "
                "scaffold_present=true; bundle_shipped=false; "
                "bundle_ship_allowed_today=false; criteria_met_today=false; "
                "bundle_land_path_executed_today=false; wire_shipped=false; "
                "wire_ship_allowed_today=false; dual_linf_under_wire=unproven; "
                "dual_linf_proof_allowed_today=false; online_linf_gate=open; "
                "gate_flip_allowed_today=false; feature_flag_enabled_today=false; "
                "first_blocking still isolation; is_first_blocking_coreq=false; "
                "companion_not_order_hint_primary=true; "
                "scaffold≠bundle shipped ≠ bundle allow ≠ land executed ≠ wire shipped ≠ "
                "dual_linf proven ≠ gate flip ≠ VERDICT; dual_recovery_path=None; "
                "PRIMARY online λ owns dual gate language"
            ),
            "actual": (
                "static honesty — offline_tf_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold_ready=true; "
                "scaffold formalized ≠ bundle shipped; bundle_ship_allowed_today remains false; "
                "land path not executed; dual_linf stays unproven; gate stays open; "
                "packaging alone ≠ bundle ship; distinct from bundle design / criteria / "
                "bundle operational prep / wire fifth scaffold packaging"
            ),
        ...
        {
            "check": "offline_tf_dual_linf_execution_scaffold_not_dual_linf_proven",
            "predicted": (
                "dual_linf fourth-coreq execution scaffold packaging present; "
                "scaffold_present=true; dual_linf_under_wire=unproven; "
                "dual_linf_proof_allowed_today=false; criteria_met_today=false; "
                "online_linf_gate=open; gate_flip_allowed_today=false; "
                "proof_composition_path_executed_today=false; form=classic_2block_excel_path; "
                "form_label_change_shipped=false; feature_flag_enabled_today=false; "
                "first_blocking still isolation; order_hint_index=3; is_first_blocking_coreq=false; "
                "scaffold≠dual_linf proven ≠ proof allow ≠ gate flip ≠ form ship ≠ wire ≠ VERDICT; "
                "dual_recovery_path=None; PRIMARY online λ owns dual gate language"
            ),
            "actual": (
                "static honesty — offline_tf_case1_dual_linf_fourth_coreq_execution_scaffold_ready=true; "
                "scaffold formalized ≠ dual_linf proven; dual_linf_proof_allowed remains false; "
                "gate stays open; gate_flip_allowed remains false; proof composition not executed; "
                "packaging alone ≠ dual_linf proven; distinct from dual_linf criteria / dual_linf prep / "
                "online_linf_gate criteria / form scaffold / isolation scaffold packaging"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        {
            "check": "offline_tf_form_label_second_coreq_prep_not_form_ship",
            "predicted": (
                "form_label second-coreq operational prep packaging present; prep_present=true; "
                "form=classic_2block_excel_path; form_label_change_shipped=false; "
                "form_label_ship_allowed_today=false; criteria_met_today=false; "
                "form_mutation_path_executed_today=false; first_blocking still isolation; "
                "prep≠form ship ≠ form flip ≠ wire ≠ VERDICT; dual_recovery_path=None"
            ),
            "actual": (
                "static honesty — offline_tf_case1_form_label_second_coreq_operational_prep_ready=true; "
                "prep formalized ≠ form_label_change_shipped; form remains classic; "
                "mutation path named not executed; packaging alone ≠ form flip; "
                "distinct from form_label ship criteria packaging"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        ...
        {
            "check": "offline_tf_case1_dual_honest_multi_blocker_wire_implementation_blueprint_not_duals",
            "predicted": (
                "offline Case-1 dual-honest multi-blocker wire *implementation blueprint* "
                "readiness packaging exists (static); blueprint_present=true; "
                "implementation_blueprint_present=true; wire_go_board_present=true; "
                f"first_blocking_coreq={_CASE1_FIRST_BLOCKING_COREQ}; "
                "rehearsal_present=true; scaffold_present=true; path_shipped=false; "
                "dual_honest_tf_aware_path_present ship-met=false; wire_shipped=false; "
                "bundle_shipped=false; bundle_ship_allowed_today=false; "
                "criteria_met_today=false; isolation_rewrite_shipped=false; "
                f"isolation_rewrite_with_wire={_CASE1_ISOLATION_REWRITE_CHECKLIST_STATUS}; "
                ...
                "wire_ship_allowed_today=false; "
                f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
                "dual_recovery_path=None on blueprint surface; package dual "
                "gate remains online_lambda; blueprint packaging is not dual recovery"
            ),
            "actual": (
                "static honesty — wire blueprint packaging dual_recovery_path=None; "
                "PRIMARY online_lambda still owns Case 1 dual gate; dual_linf under wire "
                "unproven; planned dual_recovery labeled honestly (not pure-ADMM); "
                "order_hint not executor; blueprint_present ≠ ship; first_blocking alone ≠ ship"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        {
            "check": "offline_tf_case1_dual_honest_multi_blocker_wire_implementation_blueprint_not_wire",
            "predicted": (
                "wire_shipped=False; wire_ship_allowed_today=false; path_shipped=false; "
                "bundle_shipped=false; bundle_ship_allowed_today=false; "
                "criteria_met_today=false; "
            ...
            "actual": (
                "static honesty — wire blueprint packaging only; path not shipped; "
                "bundle not shipped; ship not allowed; criteria not met; isolation rewrite "
                "not shipped; form classic; wire not shipped; blockers still true "
                "(incl. isolation_rewrite_required + no_blender_offline_affine_kernel + "
                "form_label_change_required + wire_not_shipped); dual L∞ under wire not "
                "proven; order_hint not executor / not auto-wire; first_blocking alone ≠ ship"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        {
            "check": "offline_tf_case1_dual_honest_multi_blocker_wire_implementation_blueprint_not_verdict_gate",
            "predicted": (
                "wire blueprint packaging ≠ Case 1 VERDICT dual gate; "
                "online L∞ ≤15 gate unchanged; ready flag means harness-existence only "
                "(not path shipped; not wire shipped; not bundle shipped; "
                "not isolation rewrite shipped; not form flip; not ship-met; not ship "
                "allowed; not criteria_met; not L∞≤15 under wire); "
            ...
            "actual": (
                "static honesty — wire blueprint packaging is not VERDICT dual gate; "
                "online_lambda remains PRIMARY gate; path_shipped=false; "
                "bundle_shipped=false; bundle_ship_allowed=false; criteria_met=false; "
                "isolation_rewrite_shipped=false; form classic; "
                "form_label_change_shipped=false; ship-met=false; wire_shipped=false; "
                "blueprint packaging is not Case 1 dual PASS/FAIL"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        {
            "check": "offline_tf_case1_dual_honest_multi_blocker_wire_implementation_blueprint_ship_flags_false",
            "predicted": (
                "blueprint_present=true; implementation_blueprint_present=true; "
                "wire_go_board_present=true; "
                f"first_blocking_coreq={_CASE1_FIRST_BLOCKING_COREQ}; "
                "rehearsal_present=true; scaffold_present=true; "
                "path_shipped=false; dual_honest_tf_aware_path_present ship-met=false; "
                "wire_shipped=false; bundle_shipped=false; "
                "bundle_ship_allowed_today=false; criteria_met_today=false; "
                "order_hint_is_not_executor=true; no_auto_wire=true; "
            ...
            "actual": (
                "static honesty — blueprint packaging present only; path not shipped; "
                "bundle not shipped; ship_allowed false; criteria_met false; "
                "order_hint not executor; no auto-wire; isolation rewrite not shipped; "
                "checklist open; form classic; form_label not shipped; wire not shipped; "
                "packaging ≠ path ship ≠ wire ship ≠ bundle ship ≠ isolation rewrite "
                "ship ≠ form flip ≠ ship-met; first_blocking alone ≠ ship"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
        {
            "check": "offline_tf_case1_dual_honest_multi_blocker_wire_implementation_blueprint_not_dual_linf_under_wire_proof",
            "predicted": (
                f"dual_linf_under_wire={_CASE1_DUAL_LINF_UNDER_WIRE_STATUS}; "
                "blueprint_is_not_dual_linf_under_wire_proof=true; "
                "first_blocking_coreq_alone_is_not_ship=true; "
            ...
            "actual": (
                "static honesty — dual_linf stays unproven; wire blueprint packaging "
                "never treats blueprint_present, first_blocking_coreq, go-board, or "
                "diagnostic stream L∞ as dual L∞ under wire proof, path ship, wire ship, "
                "bundle ship, isolation rewrite ship, form flip, ship-met, ship allow, or "
                "gate flip; order_hint is not executor"
            ),
            "abs_err": 0.0,
            "ok": True,
        },
...
def _how_to_read_rows(report: Dict[str, Any]) -> list[tuple[str, str]]:
    ...
    return [
        ...
        (
            "tf_offline_case1_dual_linf_fourth_coreq_execution_scaffold",
            tf_dual_linf_scaffold["planner_one_liner"],
        ),
        (
            "tf_offline_case1_wire_fifth_coreq_execution_scaffold",
            tf_wire_scaffold["planner_one_liner"],
        ),
        (
            "tf_offline_case1_dual_honest_multi_blocker_wire_bundle_execution_scaffold",
            tf_bundle_scaffold["planner_one_liner"],
        ),
        (
            "tf_offline_case1_dual_linf_under_wire_criteria_contract",
            tf_dual_linf_criteria["planner_one_liner"],
        ),
        (
            "solve_boundary",
